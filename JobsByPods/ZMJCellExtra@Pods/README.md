...
> 这份自述用于记录 `ZMJCellExtra` 在 Jobs 本地 [**CocoaPods**](https://cocoapods.org/) 体系里的职责边界、目录结构、依赖关系和验证方式。
...
| 版本 | `1.0.0` |
| 平台 | `ios 12.0` |
| 摘要 | ZMJGanttChart cell helpers for Jobs. |
| 首页 | [https://example.local/ZMJCellExtra](https://example.local/ZMJCellExtra) |
| 许可证 | `MIT / LICENSE` |
...
- 为 `ZMJCell` 或同名上游能力提供 Jobs 项目内的分类、桥接、资源或边界适配。
- 当 `ZMJCellExtra` 的 `Core`、`Support`、资源、依赖或公开头文件发生变化时，同步更新本 README，避免后续排查只看源码不看边界。
...
```text
ZMJCellExtra@Pods/
├── ZMJCellExtra.podspec  # Pod 描述文件
├── README.md  # 当前自述
├── ZMJCellExtra.h  # 根入口头文件
├── Core/  # 公开 API 与核心实现，2 个文件
└── LICENSE  # 许可证文件
```

- `ZMJCellExtra.podspec` 是当前 Pod 的 [**CocoaPods**](https://cocoapods.org/) 描述入口。
...
- `Core` 当前包含 2 个文件，其中源码 / 头文件 2 个；按 Jobs 规范，它是 `ZMJCellExtra` 对外公开 API 和核心实现的边界。
...
```objc
#if __has_include(<ZMJCellExtra/ZMJCellExtra.h>)
#import <ZMJCellExtra/ZMJCellExtra.h>
#else
#import "ZMJCellExtra.h"
#endif
```

- 自建 Pod 对外优先引用公共入口头，不要绕开聚合头直接引用 `Support` 内部子头。
- 如果 `ZMJCellExtra.h` 不是最终公开入口，请先修正 `ZMJCellExtra.podspec` 的 `public_header_files` 和入口头设计，再修改调用方。
...
修改 `ZMJCellExtra` 后，优先按风险从低到高验证：

```shell
ruby -c ZMJCellExtra.podspec
```

```shell
pod lib lint ZMJCellExtra.podspec --allow-warnings --verbose
```
