...
@implementation SRWebSocket (JobsSocketRocketUtilityDSL)
-(JobsRetSRWebSocketByDelegateBlock _Nonnull)byDelegate{
    @jobs_weakify(self)
    return ^__kindof SRWebSocket *_Nullable(id<SRWebSocketDelegate> _Nullable data){
        @jobs_strongify(self)
        self.delegate = data;
        return self;
    };
...
-(JobsRetSRWebSocketByVoidBlock _Nonnull)byOpen{
    @jobs_weakify(self)
    return ^__kindof SRWebSocket *_Nullable(void){
        @jobs_strongify(self)
        [self open];
        return self;
    };
...
-(jobsByVoidBlock _Nonnull)reConnect;
-(jobsByVoidBlock _Nonnull)destoryHeartBeat;
-(jobsByVoidBlock _Nonnull)initHeartBeat;
-(jobsByVoidBlock _Nonnull)ping;
...
+(JobsRetSocketRocketUtilityByVoidBlock _Nonnull)instance {
    return ^SocketRocketUtility *_Nullable{
...
-(jobsByStrBlock _Nonnull)SRWebSocketOpenWithURLString{
    ...
    return ^(__kindof NSString *_Nullable urlString){
        ...
        if (!urlString) return;
        self.byURLString(urlString)
            .bySocket(SRWebSocket.initByURLRequest(NSURLRequest.initBy(urlString.jobsUrl)));
        JobsLog(@"请求的websocket地址：%@",self.socket.url.absoluteString);
        self.socket.byDelegate(self).byOpen();/// SRWebSocketDelegate 协议，开始连接
    };
}
/// 关闭连接
- (jobsByVoidBlock _Nonnull)SRWebSocketClose {
    ...
    return ^{
        ...
        if (self.socket){
            [self.socket close];
            self.bySocket(nil).destoryHeartBeat();// 断开连接时销毁心跳
        }
    };
}
/// 发送数据
...
- (jobsByVoidBlock _Nonnull)reConnect {
    ...
    return ^{
        ...
        if (self->reConnectTime > 64) return;/// 您的网络状况不是很好，请检查网络后重试
        @jobs_weakify(self)
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(self->reConnectTime * NSEC_PER_SEC)),
                       dispatch_get_main_queue(), ^{
            @jobs_strongify(self)
            self.bySocket(nil).SRWebSocketOpenWithURLString(self.urlString);
            JobsLog(@"重连");
        });
        /// 重连时间2的指数级增长
...
-(void)webSocket:(SRWebSocket *)webSocket didFailWithError:(NSError *)error {
    if (webSocket == self.socket) {
        JobsLog(@"************************** socket 连接失败************************** ");
        self.bySocket(nil).reConnect();/// 连接失败就重连
    }
}
...
-(JobsRetSocketRocketUtilityByStrBlock _Nonnull)byURLString{
    @jobs_weakify(self)
    return ^__kindof SocketRocketUtility *_Nullable(NSString *_Nullable data){
        @jobs_strongify(self)
        self.urlString = data;
        return self;
    };
...
-(JobsRetSocketRocketUtilityBySRWebSocketBlock _Nonnull)bySocket{
    @jobs_weakify(self)
    return ^__kindof SocketRocketUtility *_Nullable(SRWebSocket *_Nullable data){
        @jobs_strongify(self)
        self.socket = data;
        return self;
    };
