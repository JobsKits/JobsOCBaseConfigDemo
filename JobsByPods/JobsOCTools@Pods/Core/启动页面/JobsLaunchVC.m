...
static __kindof UIViewController *_Nullable JobsLaunchVCMainViewController(void) {
    Class appDelegateClass = NSClassFromString(@"AppDelegate");
    if (!appDelegateClass) return nil;
    @try {
        id value = [appDelegateClass valueForKey:@"jobsCustomTabBarNavCtrl"];
...
//            self.switchToMainVC(FMHomeWebVC.new.navCtrl);
            __kindof UIViewController *mainViewController = JobsLaunchVCMainViewController();
            if (mainViewController) self.switchToMainVC(mainViewController);
        });
    };
}
