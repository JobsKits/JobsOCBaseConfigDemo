...
```
    ...
    @jobs_weakify(self)
    [tfView actionObjBlock:^(ZYTextField *data) {
        @jobs_strongify(self)
        self->dropDownListView.dropDownListViewDisappear(nil);
        self->chooseView.getBtn.selected = NO;
        if ([data.placeholder isEqualToString:@"Please enter name".tr]) {
            self.inputModel.name = data.text;
        }else if ([data.placeholder isEqualToString:@"Please enter bank card number".tr]){
            self.inputModel.bankCardNum = data.text;
        }else if([data.placeholder isEqualToString:@"Please enter open account address".tr]){
    ...
    [tfView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.size.mas_equalTo(JobsAppDoorInputViewBaseStyle_5.viewSizeByModel(jobsMakeViewModel(^(__kindof UIViewModel * _Nullable data) {})));
    ...
    tfView.jobsRichViewByModel(model);
    if ([tfView.getTextField.placeholder isEqualToString:@"Please enter name".tr]) {
        tfView.getTextField.enabled = isNull(self.viewModel.textModel.text);
    }
