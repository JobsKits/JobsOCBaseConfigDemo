...
-(JobsRetNSNumberByVoidBlock _Nonnull)makeSnowflake{
    ...
    return ^NSNumber *_Nonnull{
        @jobs_strongify(self)
        if (!self) return nil;
        JobsOCSnowflake *snowflake = [JobsOCSnowflake.alloc initWithPublishMillisecond:(uint64_t)(NSDate.date.timeIntervalSince1970 * 1000)
                                                                                 IDCID:1
                                                                             machineID:1];
...
-(JobsRetStrByIntegerBlock _Nonnull)whatDayOfWeekDistanceNow{
    return ^NSString *_Nullable(NSInteger offsetDay){
        NSInteger currentWeekday = [NSCalendar.currentCalendar component:NSCalendarUnitWeekday
                                                                 fromDate:NSDate.date];//当前时间是周几？1代表周日 2代表周一 7代表周六
