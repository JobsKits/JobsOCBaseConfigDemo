# `JobsGraphicCaptcha`

![Jobs出品，必属精品](https://picsum.photos/1500/400)

[toc]

---

## 🔥 <font id=前言>前言</font> <a href="#🔚" style="font-size:17px; color:green;"><b>🔽</b></a>

> `JobsGraphicCaptcha` 是 Jobs 本地 [**CocoaPods**](https://cocoapods.org/) 体系里的图形验证码 Pod，负责字符池、随机验证码文本、大小写校验策略和验证码绘制视图。

## 一、Pod 定位 <a href="#前言" style="font-size:17px; color:green;"><b>🔼</b></a> <a href="#🔚" style="font-size:17px; color:green;"><b>🔽</b></a>

| 项目 | 内容 |
| ---- | ---- |
| Pod 名称 | `JobsGraphicCaptcha` |
| Pod 类型 | 自建本地 Pod |
| 版本 | `1.0.0` |
| 平台 | `ios 12.0` |
| 摘要 | Graphic captcha generator and view for Jobs projects. |
| podspec | `JobsByPods/JobsGraphicCaptcha@Pods/JobsGraphicCaptcha.podspec` |
| source | `{ :path => '.' }` |

## 二、适用场景 <a href="#前言" style="font-size:17px; color:green;"><b>🔼</b></a> <a href="#🔚" style="font-size:17px; color:green;"><b>🔽</b></a>

- 注册、登录、找回密码等页面需要本地图形验证码时使用。
- 支持单独数字、单独汉字、单独英文、英文大小写敏感 / 不敏感、以及数字 / 英文 / 汉字混合模式。
- 默认验证码长度为 `4`，可通过 `JobsGraphicCaptchaConfig.length` 手动调整。

## 三、目录结构 <a href="#前言" style="font-size:17px; color:green;"><b>🔼</b></a> <a href="#🔚" style="font-size:17px; color:green;"><b>🔽</b></a>

```text
JobsGraphicCaptcha@Pods/
├── JobsGraphicCaptcha.podspec
├── JobsGraphicCaptchaHeader.h
├── JobsPodspecKit.rb
├── LICENSE
├── README.md
└── Core/
    ├── JobsGraphicCaptchaConfig.h
    ├── JobsGraphicCaptchaConfig.m
    ├── JobsGraphicCaptchaGenerator.h
    ├── JobsGraphicCaptchaGenerator.m
    ├── JobsGraphicCaptchaView.h
    └── JobsGraphicCaptchaView.m
```

## 四、公开能力 <a href="#前言" style="font-size:17px; color:green;"><b>🔼</b></a> <a href="#🔚" style="font-size:17px; color:green;"><b>🔽</b></a>

- `JobsGraphicCaptchaConfig`：配置验证码长度、字符单元、大小写校验策略和自定义字符池。
- `JobsGraphicCaptchaGenerator`：提供数字、小写英文、大写英文、汉字四个独立字符池，并生成随机文本。
- `JobsGraphicCaptchaView`：绘制验证码文本、干扰线和噪点，支持点击刷新和输入校验。

## 五、引用方式 <a href="#前言" style="font-size:17px; color:green;"><b>🔼</b></a> <a href="#🔚" style="font-size:17px; color:green;"><b>🔽</b></a>

```objc
#if __has_include(<JobsGraphicCaptcha/JobsGraphicCaptchaHeader.h>)
#import <JobsGraphicCaptcha/JobsGraphicCaptchaHeader.h>
#else
#import "JobsGraphicCaptchaHeader.h"
#endif
```

```objc
JobsGraphicCaptchaView *captchaView = JobsGraphicCaptchaView.new;
captchaView.config = JobsGraphicCaptchaConfig.mixedConfig;
[captchaView refreshCaptcha];
BOOL passed = [captchaView validateInput:@"Ab3中"];
```

## 六、依赖关系 <a href="#前言" style="font-size:17px; color:green;"><b>🔼</b></a> <a href="#🔚" style="font-size:17px; color:green;"><b>🔽</b></a>

- 系统框架：`Foundation`、`UIKit`、`QuartzCore`
- 本地 Pod：`JobsOCDefs`

## 七、验证方式 <a href="#前言" style="font-size:17px; color:green;"><b>🔼</b></a> <a href="#🔚" style="font-size:17px; color:green;"><b>🔽</b></a>

```shell
ruby -c JobsGraphicCaptcha.podspec
```

```shell
pod install --no-repo-update
```

- 改动 `Core`、podspec、依赖或公开头后，需要重新执行 [**CocoaPods**](https://cocoapods.org/) 集成验证。
- 汉字池使用 `0x4E00...0x9FA5` 的 CJK 常用汉字区间；如需要扩展到补充平面，可在 `JobsGraphicCaptchaGenerator` 中统一扩展。

<a id="🔚" href="#前言" style="font-size:17px; color:green; font-weight:bold;">我是有底线的➤点我回到首页</a>
