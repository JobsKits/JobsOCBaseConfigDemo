...
> 这份自述用于记录 `JobsStringUtils` 在 Jobs 本地 [**CocoaPods**](https://cocoapods.org/) 体系里的职责边界、目录结构、依赖关系和验证方式。
补充描述：JobsStringUtils provides NSString related utility methods.
...
| 项目 | 内容 |
| ---- | ---- |
| Pod 名称 | `JobsStringUtils` |
| Pod 类型 | 自建本地 Pod |
| 版本 | `1.0.0` |
| 平台 | `ios 12.0` |
| 摘要 | String utilities for Jobs. |
| 首页 | [https://example.local/JobsStringUtils](https://example.local/JobsStringUtils) |
| 许可证 | `MIT / LICENSE` |
...
- 作为 Jobs 项目内的独立能力 Pod，向 App 或其它 Pod 提供 `JobsStringUtils` 相关能力。
- 当 `JobsStringUtils` 的 `Core`、`Support`、资源、依赖或公开头文件发生变化时，同步更新本 README，避免后续排查只看源码不看边界。
- 参与本地 Pods 拆分时，先确认能力归属，再决定放入当前 Pod、迁移到 `Support`，还是下沉为更基础的公共 Pod。

## 三、目录结构 <a href="#前言" style="font-size:17px; color:green;"><b>🔼</b></a> <a href="#🔚" style="font-size:17px; color:green;"><b>🔽</b></a>

```text
JobsStringUtils@Pods/
├── JobsStringUtils.podspec  # Pod 描述文件
├── JobsStringUtilsHeader.h  # 根聚合头文件
├── README.md  # 当前自述
...
- `JobsStringUtils.podspec` 是当前 Pod 的 [**CocoaPods**](https://cocoapods.org/) 描述入口。
- `README.md` 是当前文件，负责说明用途、边界、依赖、资源和风险。
- 若目录中存在 `JobsPodspecKit.rb`，说明该 Pod 使用 Jobs 本地 podspec 基座维护通用 exclude / xcconfig 规则。

## 四、`Core` / `Support` 边界 <a href="#前言" style="font-size:17px; color:green;"><b>🔼</b></a> <a href="#🔚" style="font-size:17px; color:green;"><b>🔽</b></a>

- `Core` 当前包含 2 个文件，其中源码 / 头文件 2 个；按 Jobs 规范，它是 `JobsStringUtils` 对外公开 API 和核心实现的边界。
- `Support` 当前未启用；`JobsStringUtils` 处在 `JobsBlock` 上游链路中，不能为了 DSL 反向依赖 `JobsBlock`。
...
- `JobsStringUtilsHeader.h` 是统一公开入口；调用方不绕开聚合头引用 `Core` 内部子头。
...
修改 `JobsStringUtils` 后，优先按风险从低到高验证：

```shell
ruby -c JobsStringUtils.podspec
```

```shell
pod lib lint JobsStringUtils.podspec --allow-warnings --verbose
```
