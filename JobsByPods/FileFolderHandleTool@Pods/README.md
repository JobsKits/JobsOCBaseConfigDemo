...
| 项目 | 内容 |
| ---- | ---- |
| Pod 名称 | `FileFolderHandleTool` |
| Pod 类型 | 自建本地 Pod |
| 版本 | `1.0.0` |
| 平台 | `ios 12.0` |
| 摘要 | Objective-C file and folder handle tool for Jobs projects. |
| 首页 | [https://example.local/FileFolderHandleTool](https://example.local/FileFolderHandleTool) |
| 许可证 | `MIT / LICENSE` |
...
- 作为 Jobs 项目内的独立能力 Pod，向 App 或其它 Pod 提供 `FileFolderHandleTool` 相关能力。
- 当 `FileFolderHandleTool` 的 `Core`、`Support`、资源、依赖或公开头文件发生变化时，同步更新本 README，避免后续排查只看源码不看边界。
- 参与本地 Pods 拆分时，先确认能力归属，再决定放入当前 Pod、迁移到 `Support`，还是下沉为更基础的公共 Pod。

## 三、目录结构 <a href="#前言" style="font-size:17px; color:green;"><b>🔼</b></a> <a href="#🔚" style="font-size:17px; color:green;"><b>🔽</b></a>

```text
FileFolderHandleTool@Pods/
├── FileFolderHandleTool.podspec  # Pod 描述文件
├── FileFolderHandleToolHeader.h  # 根聚合头文件
├── README.md  # 当前自述
...
- `FileFolderHandleTool.podspec` 是当前 Pod 的 [**CocoaPods**](https://cocoapods.org/) 描述入口。
...
- `Core` 当前包含 2 个文件，其中源码 / 头文件 2 个；按 Jobs 规范，它是 `FileFolderHandleTool` 对外公开 API 和核心实现的边界。
...
- `FileFolderHandleToolHeader.h` 是统一公开入口；调用方不绕开聚合头引用 `Core` 内部子头。
...
修改 `FileFolderHandleTool` 后，优先按风险从低到高验证：

```shell
ruby -c FileFolderHandleTool.podspec
```

```shell
pod lib lint FileFolderHandleTool.podspec --allow-warnings --verbose
```
...
- [Core/FileFolderHandleTool/FileFolderHandleTool.h](<./Core/FileFolderHandleTool/FileFolderHandleTool.h>)
- [FileFolderHandleToolHeader.h](<./FileFolderHandleToolHeader.h>)

依赖与编译入口：[FileFolderHandleTool.podspec](<./FileFolderHandleTool.podspec>)。其中显式依赖声明包括 `JobsModelDSL`、`JobsMakes`、`JobsBlock`、`JobsOCDefs`、`JobsByOCPods`、`TXFileOperation`。源码范围、资源及可选 subspec 以这里的声明为准；辅助脚本动态补充的依赖不在上述摘录中展开。
