...
```objc
#if __has_include(<JobsUploadingProgressView/JobsUploadingProgressView.h>)
#import <JobsUploadingProgressView/JobsUploadingProgressView.h>
#else
...
- 如果 `JobsUploadingProgressView.h` 不是最终公开入口，请先修正 `JobsUploadingProgressView.podspec` 的 `public_header_files` 和入口头设计，再修改调用方。
