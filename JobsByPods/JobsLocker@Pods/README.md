...
| 版本 | `1.0.0` |
| 平台 | `ios 12.0` |
| 摘要 | A lightweight Objective-C locking utility with a per-instance once extension. |
| 首页 | [https://github.com/JobsKits/JobsLocker](https://github.com/JobsKits/JobsLocker) |
| 许可证 | `MIT / LICENSE` |
...
- 作为 Jobs 项目内的独立能力 Pod，向 App 或其它 Pod 提供 `JobsLocker` 相关能力。
- 当 `JobsLocker` 的 `Core`、`Support`、资源、依赖或公开头文件发生变化时，同步更新本 README，避免后续排查只看源码不看边界。
...
- `JobsLocker.podspec` 是当前 Pod 的 [**CocoaPods**](https://cocoapods.org/) 描述入口。
...
- `Core` 当前包含 4 个文件，其中源码 / 头文件 4 个；按 Jobs 规范，它是 `JobsLocker` 对外公开 API 和核心实现的边界。
...
```objc
#if __has_include(<JobsLocker/JobsLockerUmbrella.h>)
#import <JobsLocker/JobsLockerUmbrella.h>
#else
#import "JobsLockerUmbrella.h"
#endif
```

- 自建 Pod 对外优先引用公共入口头，不要绕开聚合头直接引用 `Support` 内部子头。
- 如果 `JobsLockerUmbrella.h` 不是最终公开入口，请先修正 `JobsLocker.podspec` 的 `public_header_files` 和入口头设计，再修改调用方。
...
修改 `JobsLocker` 后，优先按风险从低到高验证：

```shell
ruby -c JobsLocker.podspec
```

```shell
pod lib lint JobsLocker.podspec --allow-warnings --verbose
```
