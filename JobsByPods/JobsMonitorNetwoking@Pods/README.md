...
> 这份自述用于记录 `JobsMonitorNetwoking` 在 Jobs 本地 [**CocoaPods**](https://cocoapods.org/) 体系里的职责边界、目录结构、依赖关系和验证方式。
补充描述：JobsMonitorNetwoking is a local Objective-C component that calculates current network traffic speed and displays it with JobsSuspendLab.
...
| 项目 | 内容 |
| ---- | ---- |
| Pod 名称 | `JobsMonitorNetwoking` |
| Pod 类型 | 自建本地 Pod |
| 版本 | `1.0.0` |
| 平台 | `ios 12.0` |
| 摘要 | Objective-C network speed monitoring component for Jobs projects. |
| 首页 | [https://example.local/JobsMonitorNetwoking](https://example.local/JobsMonitorNetwoking) |
| 许可证 | `MIT / LICENSE` |
...
- 作为 Jobs 项目内的独立能力 Pod，向 App 或其它 Pod 提供 `JobsMonitorNetwoking` 相关能力。
- 当 `JobsMonitorNetwoking` 的 `Core`、`Support`、资源、依赖或公开头文件发生变化时，同步更新本 README，避免后续排查只看源码不看边界。
- 参与本地 Pods 拆分时，先确认能力归属，再决定放入当前 Pod、迁移到 `Support`，还是下沉为更基础的公共 Pod。

## 三、目录结构 <a href="#前言" style="font-size:17px; color:green;"><b>🔼</b></a> <a href="#🔚" style="font-size:17px; color:green;"><b>🔽</b></a>

```text
JobsMonitorNetwoking@Pods/
├── JobsMonitorNetwoking.podspec  # Pod 描述文件
├── JobsMonitorNetwokingHeader.h  # 根聚合头文件
├── README.md  # 当前自述
...
- `JobsMonitorNetwoking.podspec` 是当前 Pod 的 [**CocoaPods**](https://cocoapods.org/) 描述入口。
...
- `Core` 当前包含 2 个文件，其中源码 / 头文件 2 个；按 Jobs 规范，它是 `JobsMonitorNetwoking` 对外公开 API 和核心实现的边界。
...
```objc
#if __has_include(<JobsMonitorNetwoking/JobsMonitorNetwokingHeader.h>)
#import <JobsMonitorNetwoking/JobsMonitorNetwokingHeader.h>
#else
#import "JobsMonitorNetwokingHeader.h"
#endif
```

- 自建 Pod 对外优先引用公共入口头，不要绕开聚合头直接引用 `Support` 内部子头。
- `JobsMonitorNetwokingHeader.h` 是统一公开入口；调用方不绕开聚合头引用 `Core` 内部子头。
...
修改 `JobsMonitorNetwoking` 后，优先按风险从低到高验证：

```shell
ruby -c JobsMonitorNetwoking.podspec
```

```shell
pod lib lint JobsMonitorNetwoking.podspec --allow-warnings --verbose
```
