//
//  CAScrollLayer+DSL.m
//  JobsOCDSL
//
//  Created by Jobs on 2026年6月8日，星期一.
//

#import "CAScrollLayer+DSL.h"

@implementation CAScrollLayer (JobsChain)
#pragma mark —— Inherited CALayer DSL
#pragma mark —— Geometry
#pragma mark —— Hierarchy
#pragma mark —— Display
#pragma mark —— Visibility
#pragma mark —— Border / Shadow
#pragma mark —— Filters / Rasterize
#pragma mark —— Layout / Action
#pragma mark —— CAScrollLayer
-(JobsRetCAScrollLayerByScrollModeBlock _Nonnull)byScrollMode{
    @jobs_weakify(self)
    return ^__kindof CAScrollLayer *_Nullable(CAScrollLayerScrollMode data){
        @jobs_strongify(self)
        self.scrollMode = data;
        return self;
    };
}

-(JobsRetCAScrollLayerByCGPointBlock _Nonnull)byScrollToPoint{
    @jobs_weakify(self)
    return ^__kindof CAScrollLayer *_Nullable(CGPoint data){
        @jobs_strongify(self)
        [self scrollToPoint:data];
        return self;
    };
}

-(JobsRetCAScrollLayerByCGRectBlock _Nonnull)byScrollToRect{
    @jobs_weakify(self)
    return ^__kindof CAScrollLayer *_Nullable(CGRect data){
        @jobs_strongify(self)
        [self scrollToRect:data];
        return self;
    };
}

@end
