...
| 版本 | `1.0.0` |
| 平台 | `ios 12.0` |
| 摘要 | This base object for Jobs Objective-C projects. |
| 首页 | [https://example.local/This](https://example.local/This) |
| 许可证 | `MIT / LICENSE` |
...
- 作为 Jobs 项目内的独立能力 Pod，向 App 或其它 Pod 提供 `This` 相关能力。
- 当 `This` 的 `Core`、`Support`、资源、依赖或公开头文件发生变化时，同步更新本 README，避免后续排查只看源码不看边界。
...
- `This.podspec` 是当前 Pod 的 [**CocoaPods**](https://cocoapods.org/) 描述入口。
...
- `Core` 当前包含 2 个文件，其中源码 / 头文件 2 个；按 Jobs 规范，它是 `This` 对外公开 API 和核心实现的边界。
...
- `ThisHeader.h` 是统一公开入口；调用方不绕开聚合头引用 `Core` 内部子头。
...
```shell
pod lib lint This.podspec --allow-warnings --verbose
```
...
- [Core/This/This.h](<./Core/This/This.h>)
- [ThisHeader.h](<./ThisHeader.h>)

依赖与编译入口：[This.podspec](<./This.podspec>)。源码范围、资源及可选 subspec 以这里的声明为准；辅助脚本动态补充的依赖不在上述摘录中展开。
