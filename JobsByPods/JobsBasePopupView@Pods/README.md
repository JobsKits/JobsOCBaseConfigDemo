...
> 这份自述用于记录 `JobsBasePopupView` 在 Jobs 本地 [**CocoaPods**](https://cocoapods.org/) 体系里的职责边界、目录结构、依赖关系和验证方式。
补充描述：JobsBasePopupView is a local Objective-C UI component library providing a reusable popup base view with title, subtitle, action buttons, and model-driven view rendering support.
...
| 项目 | 内容 |
| ---- | ---- |
| Pod 名称 | `JobsBasePopupView` |
| Pod 类型 | 自建本地 Pod |
| 版本 | `1.0.0` |
| 平台 | `ios 12.0` |
| 摘要 | Base popup view component for Jobs projects. |
| 首页 | [https://example.local/JobsBasePopupView](https://example.local/JobsBasePopupView) |
| 许可证 | `MIT / LICENSE` |
...
- 作为 Jobs 项目内的独立能力 Pod，向 App 或其它 Pod 提供 `JobsBasePopupView` 相关能力。
- 当 `JobsBasePopupView` 的 `Core`、`Support`、资源、依赖或公开头文件发生变化时，同步更新本 README，避免后续排查只看源码不看边界。
- 参与本地 Pods 拆分时，先确认能力归属，再决定放入当前 Pod、迁移到 `Support`，还是下沉为更基础的公共 Pod。

## 三、目录结构 <a href="#前言" style="font-size:17px; color:green;"><b>🔼</b></a> <a href="#🔚" style="font-size:17px; color:green;"><b>🔽</b></a>

```text
JobsBasePopupView@Pods/
├── JobsBasePopupView.podspec  # Pod 描述文件
├── README.md  # 当前自述
├── JobsPodspecKit.rb  # 本地 podspec 基座
├── Core/  # 公开 API 与核心实现，2 个文件
├── Support/  # 内部支撑层，99 个文件
└── LICENSE  # 许可证文件
```

- `JobsBasePopupView.podspec` 是当前 Pod 的 [**CocoaPods**](https://cocoapods.org/) 描述入口。
...
- `Core` 当前包含 2 个文件，其中源码 / 头文件 2 个；按 Jobs 规范，它是 `JobsBasePopupView` 对外公开 API 和核心实现的边界。
- `Support` 当前包含 99 个文件，其中源码 / 头文件 94 个；它只服务当前 Pod 内部实现，不建议被 App 层或其它 Pod 直接引用。
...
- 如果 `JobsBasePopupView.h` 不是最终公开入口，请先修正 `JobsBasePopupView.podspec` 的 `public_header_files` 和入口头设计，再修改调用方。
...
修改 `JobsBasePopupView` 后，优先按风险从低到高验证：

```shell
ruby -c JobsBasePopupView.podspec
```

```shell
pod lib lint JobsBasePopupView.podspec --allow-warnings --verbose
```
