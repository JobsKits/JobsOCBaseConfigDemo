...
typedef void(^jobsByContextualActionBlock)(__kindof UIContextualAction *_Nullable action);
typedef void(^jobsBySwipeActionsConfigurationBlock)(__kindof UISwipeActionsConfiguration *_Nullable config);
...
typedef void(^jobsByBarButtonItemBlock)(__kindof UIBarButtonItem *_Nullable item) API_AVAILABLE(ios(2.0)) API_UNAVAILABLE(watchos);
/// 按钮配置
...
typedef void(^jobsByGTCaptcha4SessionBlock)(GTCaptcha4Session *);
...
typedef void(^jobsByLiveChatMessageBlock)(LiveChatMessage *);
