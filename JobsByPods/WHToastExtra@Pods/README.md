...
> 这份自述用于记录 `WHToastExtra` 在 Jobs 本地 [**CocoaPods**](https://cocoapods.org/) 体系里的职责边界、目录结构、依赖关系和验证方式。
补充描述：WHToastExtra provides lightweight Objective-C category extensions around WHToast, including toast display helpers and local image path helpers.
...
| 版本 | `1.0.0` |
| 平台 | `ios 12.0` |
| 摘要 | WHToast category extensions for Jobs. |
| 首页 | [https://example.local/WHToastExtra](https://example.local/WHToastExtra) |
| 许可证 | `MIT / LICENSE` |
...
- 为 `WHToast` 或同名上游能力提供 Jobs 项目内的分类、桥接、资源或边界适配。
- 当 `WHToastExtra` 的 `Core`、`Support`、资源、依赖或公开头文件发生变化时，同步更新本 README，避免后续排查只看源码不看边界。
...
- 如果 `WHToastExtra.h` 不是最终公开入口，请先修正 `WHToastExtra.podspec` 的 `public_header_files` 和入口头设计，再修改调用方。
...
修改 `WHToastExtra` 后，优先按风险从低到高验证：

```shell
ruby -c WHToastExtra.podspec
```

```shell
pod lib lint WHToastExtra.podspec --allow-warnings --verbose
```
...
把 WHToast 的普通、成功、失败、加载和隐藏提示组织为 NSObject 的类/实例 Block 入口。调用方使用统一业务语义，真正的提示展示由 WHToast 承担。

### 10.2、运行脉络

选择提示语义 → 传入文案 → 转交 WHToast 展示 → 在需要时调用隐藏入口。

### 10.3、关键设计与边界

- 类入口和实例入口是两种使用形式，不应形成两套不一致的提示规则。
- 加载提示需要关注结束时机，不能只重建 show 而漏掉 hide。
- 文案、主题与宿主显示环境属于组合依赖，提示工具不负责判断业务成功或失败。

### 10.4、阅读与重建顺序

先对照五组提示入口，再核对它们调用上游的具体方式与显示参数。
