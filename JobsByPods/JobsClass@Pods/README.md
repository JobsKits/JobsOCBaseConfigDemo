...
| 版本 | `1.0.0` |
| 平台 | `ios 12.0` |
| 摘要 | NSObject class/runtime helpers for Jobs. |
| 首页 | [https://example.local/JobsClass](https://example.local/JobsClass) |
| 许可证 | `MIT / LICENSE` |
...
- 作为 Jobs 项目内的独立能力 Pod，向 App 或其它 Pod 提供 `JobsClass` 相关能力。
- 当 `JobsClass` 的 `Core`、`Support`、资源、依赖或公开头文件发生变化时，同步更新本 README，避免后续排查只看源码不看边界。
...
- `JobsClass.podspec` 是当前 Pod 的 [**CocoaPods**](https://cocoapods.org/) 描述入口。
...
- `Core` 当前包含 3 个文件，其中源码 / 头文件 3 个；按 Jobs 规范，它是 `JobsClass` 对外公开 API 和核心实现的边界。
...
- 如果 `JobsClass.h` 不是最终公开入口，请先修正 `JobsClass.podspec` 的 `public_header_files` 和入口头设计，再修改调用方。
...
修改 `JobsClass` 后，优先按风险从低到高验证：

```shell
ruby -c JobsClass.podspec
```

```shell
pod lib lint JobsClass.podspec --allow-warnings --verbose
```
...
- [JobsClass.h](<./JobsClass.h>)
- [Core/JobsClassInline/JobsClassInline.h](<./Core/JobsClassInline/JobsClassInline.h>)
- [Core/NSObject+Class/NSObject+Class.h](<./Core/NSObject+Class/NSObject+Class.h>)

依赖与编译入口：[JobsClass.podspec](<./JobsClass.podspec>)。其中显式依赖声明包括 `MJExtension`、`JobsBlock`、`JobsOCDefs`。源码范围、资源及可选 subspec 以这里的声明为准；辅助脚本动态补充的依赖不在上述摘录中展开。
