...
> 这份自述用于记录 `JobsMenuView` 在 Jobs 本地 [**CocoaPods**](https://cocoapods.org/) 体系里的职责边界、目录结构、依赖关系和验证方式。
补充描述：JobsMenuView is a local Objective-C UI component library that provides menu view presentation and interaction support for Jobs projects.
...
| 版本 | `1.0.0` |
| 平台 | `ios 12.0` |
| 摘要 | Menu view component for Jobs projects. |
| 首页 | [https://example.local/JobsMenuView](https://example.local/JobsMenuView) |
| 许可证 | `MIT / LICENSE` |
...
- 作为 Jobs 项目内的独立能力 Pod，向 App 或其它 Pod 提供 `JobsMenuView` 相关能力。
- 当 `JobsMenuView` 的 `Core`、`Support`、资源、依赖或公开头文件发生变化时，同步更新本 README，避免后续排查只看源码不看边界。
...
- `JobsMenuView.podspec` 是当前 Pod 的 [**CocoaPods**](https://cocoapods.org/) 描述入口。
...
- `Core` 当前包含 10 个文件，其中源码 / 头文件 10 个；按 Jobs 规范，它是 `JobsMenuView` 对外公开 API 和核心实现的边界。
...
```objc
#if __has_include(<JobsMenuView/JobsMenuSubView@1.h>)
#import <JobsMenuView/JobsMenuSubView@1.h>
#else
...
- 如果 `JobsMenuSubView@1.h` 不是最终公开入口，请先修正 `JobsMenuView.podspec` 的 `public_header_files` 和入口头设计，再修改调用方。
...
修改 `JobsMenuView` 后，优先按风险从低到高验证：

```shell
ruby -c JobsMenuView.podspec
```

```shell
pod lib lint JobsMenuView.podspec --allow-warnings --verbose
```
