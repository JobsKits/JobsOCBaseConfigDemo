...
> 这份自述用于记录 `FDFullscreenPopGesture` 在 Jobs 本地 [**CocoaPods**](https://cocoapods.org/) 体系里的职责边界、目录结构、依赖关系和验证方式。
补充描述：FDFullscreenPopGesture is a local Objective-C gesture component library that provides fullscreen pop gesture support for UINavigationController and simultaneous gesture handling support for UIScrollView.
...
| 项目 | 内容 |
| ---- | ---- |
| Pod 名称 | `FDFullscreenPopGesture` |
| Pod 类型 | 自建本地 Pod |
| 版本 | `1.0.0` |
| 平台 | `ios 12.0` |
| 摘要 | Objective-C fullscreen pop gesture components for Jobs projects. |
| 首页 | [https://example.local/FDFullscreenPopGesture](https://example.local/FDFullscreenPopGesture) |
| 许可证 | `MIT / LICENSE` |
...
- 作为 Jobs 项目内的独立能力 Pod，向 App 或其它 Pod 提供 `FDFullscreenPopGesture` 相关能力。
- 当 `FDFullscreenPopGesture` 的 `Core`、`Support`、资源、依赖或公开头文件发生变化时，同步更新本 README，避免后续排查只看源码不看边界。
...
├── FDFullscreenPopGesture.podspec  # Pod 描述文件
├── README.md  # 当前自述
├── FDFullscreenPopGesture.h  # 根入口头文件
...
- `FDFullscreenPopGesture.podspec` 是当前 Pod 的 [**CocoaPods**](https://cocoapods.org/) 描述入口。
...
- `Core` 当前包含 4 个文件，其中源码 / 头文件 4 个；按 Jobs 规范，它是 `FDFullscreenPopGesture` 对外公开 API 和核心实现的边界。
...
- `JobsOCDefs`
- `JobsBlock`
- `FDFullscreenPopGesture/Support/UIKits/NSObject`
- `FDFullscreenPopGesture/Support/UIKits/UIViewController`
...
- 如果 `FDFullscreenPopGesture.h` 不是最终公开入口，请先修正 `FDFullscreenPopGesture.podspec` 的 `public_header_files` 和入口头设计，再修改调用方。
...
修改 `FDFullscreenPopGesture` 后，优先按风险从低到高验证：

```shell
ruby -c FDFullscreenPopGesture.podspec
```

```shell
pod lib lint FDFullscreenPopGesture.podspec --allow-warnings --verbose
```
