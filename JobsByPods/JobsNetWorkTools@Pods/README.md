...
依赖与编译入口：[JobsNetWorkTools.podspec](<./JobsNetWorkTools.podspec>)。其中显式依赖声明包括 `JobsBlock`、`JobsModelDSL`、`JobsOCDefs`、`JobsOCTimer`、`JobsOCProtocols`。源码范围、资源及可选 subspec 以这里的声明为准；辅助脚本动态补充的依赖不在上述摘录中展开。
