...
| 版本 | `1.0.0` |
| 平台 | `ios 12.0` |
| 摘要 | Objective-C navigation bar components for Jobs projects. |
| 首页 | [https://example.local/JobsNavBar](https://example.local/JobsNavBar) |
| 许可证 | `MIT` |
...
- 作为 Jobs 项目内的独立能力 Pod，向 App 或其它 Pod 提供 `JobsNavBar` 相关能力。
- 当 `JobsNavBar` 的 `Core`、`Support`、资源、依赖或公开头文件发生变化时，同步更新本 README，避免后续排查只看源码不看边界。
...
```text
JobsNavBar@Pods/
├── JobsNavBar.podspec  # Pod 描述文件
├── JobsNavBarHeader.h  # 根聚合头文件
├── README.md  # 当前自述
├── JobsPodspecKit.rb  # 本地 podspec 基座
├── Core/  # 公开 API 与核心实现，4 个文件
├── Support/  # 内部支撑层，94 个文件
└── LICENSE  # 许可证文件
```

- `JobsNavBar.podspec` 是当前 Pod 的 [**CocoaPods**](https://cocoapods.org/) 描述入口。
...
- `Core` 当前包含 4 个文件，其中源码 / 头文件 4 个；按 Jobs 规范，它是 `JobsNavBar` 对外公开 API 和核心实现的边界。
- `Support` 当前包含 94 个文件，其中源码 / 头文件 94 个；它只服务当前 Pod 内部实现，不建议被 App 层或其它 Pod 直接引用。
...
- `JobsNavBarHeader.h` 是统一公开入口；调用方不绕开聚合头引用 `Core` 内部子头。
...
修改 `JobsNavBar` 后，优先按风险从低到高验证：

```shell
ruby -c JobsNavBar.podspec
```

```shell
pod lib lint JobsNavBar.podspec --allow-warnings --verbose
```
...
- [Core/JobsNavBar/JobsNavBar.h](<./Core/JobsNavBar/JobsNavBar.h>)
- [Core/JobsNavBarConfig/JobsNavBarConfig.h](<./Core/JobsNavBarConfig/JobsNavBarConfig.h>)
- [JobsNavBarHeader.h](<./JobsNavBarHeader.h>)

依赖与编译入口：[JobsNavBar.podspec](<./JobsNavBar.podspec>)。其中显式依赖声明包括 `Masonry`、`MJRefresh`、`MJExtension`、`JobsMakes`、`JobsBlock`、`JobsOCDSL`、`JobsModelDSL`、`JobsOCDefs`、`JobsAppTools`、`JobsDeviceInfo`、`JobsOCRuntimeKits`、`JobsRichTextUtils`、`JobsStringUtils`、`JobsOCProtocols`、`JobsLanMgr`、`MJRefreshExtra`、`ReactiveObjC`、`TABAnimated`、`WHToastExtra`、`XYColorOC`、`XZMRefresh`、`lottie-ios`。源码范围、资源及可选 subspec 以这里的声明为准；辅助脚本动态补充的依赖不在上述摘录中展开。
