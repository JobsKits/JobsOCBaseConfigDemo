...
+(JobsRetStrByStrBlock _Nonnull)remoteVideoWiFiDownloadNoticeWithLanguageCode{
    return ^NSString *(NSString * languageCode){
        NSBundle *bundle = self.localizedBundleWithLanguageCode(languageCode);
...
+(JobsRetBundleByStrBlock _Nonnull)localizedBundleWithLanguageCode{
    return ^NSBundle *(NSString * languageCode){
...
+(JobsRetNSArrayNSStringByNSStringBlock _Nonnull)languageCandidates{
    return ^NSArray<NSString *> *(NSString * rawCode){
