...
-(JobsRetStrByCorBlock _Nonnull)rgbCorStrBy{
    ...
    return ^NSString *_Nullable(UIColor * _Nullable data) {
        ...
        return [NSString stringWithFormat:@"红色:%d绿色:%d蓝色:%d",
                (int)(red * 255),
                (int)(green * 255),
                (int)(blue * 255)];
    };
...
-(JobsRetCorModelByVoidBlock _Nonnull)hexadecimalCorStrBy{
    ...
    return ^JobsCorModel *_Nullable(void) {
        ...
        Class modelClass = NSClassFromString(@"JobsCorModel");
        id data = modelClass.new;
        [data setValue:@(redCor * 255) forKey:@"red"];
        [data setValue:@(greenCor * 255) forKey:@"green"];
        [data setValue:@(blueCor * 255) forKey:@"blue"];
        [data setValue:@(CGColorGetAlpha(self.CGColor)) forKey:@"alpha"];
        return data;
    };
}
/// iOS 父视图透明度影响到子视图
