...
| 项目 | 内容 |
| ---- | ---- |
| Pod 名称 | `JobsLinkageMenuView` |
| Pod 类型 | 自建本地 Pod |
| 版本 | `1.0.0` |
| 平台 | `ios 12.0` |
| 摘要 | Linkage menu view component for Jobs projects. |
| 首页 | [https://example.local/JobsLinkageMenuView](https://example.local/JobsLinkageMenuView) |
| 许可证 | `MIT / LICENSE` |
...
├── Support/  # 内部支撑层，84 个文件
└── LICENSE  # 许可证文件
```

- `JobsLinkageMenuView.podspec` 是当前 Pod 的 [**CocoaPods**](https://cocoapods.org/) 描述入口。
...
- `Core` 当前包含 4 个文件，其中源码 / 头文件 4 个；按 Jobs 规范，它是 `JobsLinkageMenuView` 对外公开 API 和核心实现的边界。
- `Support` 当前包含 84 个文件，其中源码 / 头文件 82 个；它只服务当前 Pod 内部实现，不建议被 App 层或其它 Pod 直接引用。
...
修改 `JobsLinkageMenuView` 后，优先按风险从低到高验证：

```shell
ruby -c JobsLinkageMenuView.podspec
```

```shell
pod lib lint JobsLinkageMenuView.podspec --allow-warnings --verbose
```
