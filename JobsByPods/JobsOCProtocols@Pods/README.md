...
| 项目 | 内容 |
| ---- | ---- |
| Pod 名称 | `JobsOCProtocols` |
| Pod 类型 | 自建本地 Pod |
| 版本 | `1.0.0` |
| 平台 | `ios 12.0` |
| 摘要 | Protocols definitions for Jobs. |
| 首页 | [https://example.local/JobsOCProtocols](https://example.local/JobsOCProtocols) |
| 许可证 | `MIT / LICENSE` |
...
- 作为 Jobs 项目内的独立能力 Pod，向 App 或其它 Pod 提供 `JobsOCProtocols` 相关能力。
- 当 `JobsOCProtocols` 的 `Core`、`Support`、资源、依赖或公开头文件发生变化时，同步更新本 README，避免后续排查只看源码不看边界。
- 参与本地 Pods 拆分时，先确认能力归属，再决定放入当前 Pod、迁移到 `Support`，还是下沉为更基础的公共 Pod。

## 三、目录结构 <a href="#前言" style="font-size:17px; color:green;"><b>🔼</b></a> <a href="#🔚" style="font-size:17px; color:green;"><b>🔽</b></a>

```text
JobsOCProtocols@Pods/
├── JobsOCProtocols.podspec  # Pod 描述文件
├── README.md  # 当前自述
├── JobsPodspecKit.rb  # 本地 podspec 基座
├── Core/  # 公开 API 与核心实现，30 个文件
└── LICENSE  # 许可证文件
```

- `JobsOCProtocols.podspec` 是当前 Pod 的 [**CocoaPods**](https://cocoapods.org/) 描述入口。
...
- `Core` 当前包含 30 个文件，其中源码 / 头文件 30 个；按 Jobs 规范，它是 `JobsOCProtocols` 对外公开 API 和核心实现的边界。
...
- 如果 `JobsBaseProtocolHeader.h` 不是最终公开入口，请先修正 `JobsOCProtocols.podspec` 的 `public_header_files` 和入口头设计，再修改调用方。
...
修改 `JobsOCProtocols` 后，优先按风险从低到高验证：

```shell
ruby -c JobsOCProtocols.podspec
```

```shell
pod lib lint JobsOCProtocols.podspec --allow-warnings --verbose
```
...
- [JobsBaseProtocolHeader.h](<./JobsBaseProtocolHeader.h>)
- [Core/BaseProtocols/AppToolsProtocol/AppToolsProtocol.h](<./Core/BaseProtocols/AppToolsProtocol/AppToolsProtocol.h>)
- [Core/BaseProtocols/BaseProtocol/BaseProtocol.h](<./Core/BaseProtocols/BaseProtocol/BaseProtocol.h>)
- [Core/BaseProtocols/GestureProtocol/GestureProtocol.h](<./Core/BaseProtocols/GestureProtocol/GestureProtocol.h>)
- [Core/BaseProtocols/TimerProtocol/TimerProtocol.h](<./Core/BaseProtocols/TimerProtocol/TimerProtocol.h>)

依赖与编译入口：[JobsOCProtocols.podspec](<./JobsOCProtocols.podspec>)。其中显式依赖声明包括 `JobsBlock`、`JobsOCDefs`、`SPAlertController`、`ReactiveObjC`、`YTKNetwork`。源码范围、资源及可选 subspec 以这里的声明为准；辅助脚本动态补充的依赖不在上述摘录中展开。
