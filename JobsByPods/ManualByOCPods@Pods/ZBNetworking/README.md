...
> 这份自述用于记录 `ZBNetworking` 在 Jobs 本地 [**CocoaPods**](https://cocoapods.org/) 体系里的职责边界、目录结构、依赖关系和验证方式。
补充描述：ZBNetworking is a lightweight networking library based on AFNetworking with request management and caching support.
...
| 版本 | `1.0.4` |
| 平台 | `ios 12.0` |
| 摘要 | The network request library adds caching policies. |
| 首页 | [https://github.com/Suzhibin/ZBNetworking](https://github.com/Suzhibin/ZBNetworking) |
| 许可证 | `MIT / LICENSE` |
| 作者 | `Suzhibin / szb2323@163.com` |
...
- 当 `ZBNetworking` 的 `Core`、`Support`、资源、依赖或公开头文件发生变化时，同步更新本 README，避免后续排查只看源码不看边界。
...
```text
ZBNetworking/
├── ZBNetworking.podspec  # Pod 描述文件
├── README.md  # 当前自述
├── ZBNetworking.h  # 根入口头文件
├── Core/  # 公开 API 与核心实现，9 个文件
├── Support/  # 内部支撑层，2 个文件
└── LICENSE  # 许可证文件
```

- `ZBNetworking.podspec` 是当前 Pod 的 [**CocoaPods**](https://cocoapods.org/) 描述入口。
...
- `Core` 当前包含 9 个文件，其中源码 / 头文件 9 个；按 Jobs 规范，它是 `ZBNetworking` 对外公开 API 和核心实现的边界。
...
- 如果 `ZBNetworking.h` 不是最终公开入口，请先修正 `ZBNetworking.podspec` 的 `public_header_files` 和入口头设计，再修改调用方。
...
修改 `ZBNetworking` 后，优先按风险从低到高验证：

```shell
ruby -c ZBNetworking.podspec
```

```shell
pod lib lint ZBNetworking.podspec --allow-warnings --verbose
```
