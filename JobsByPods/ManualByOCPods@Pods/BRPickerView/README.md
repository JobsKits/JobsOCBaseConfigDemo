...
| 版本 | `3.0.0` |
| 平台 | `ios 11.0` |
| 摘要 | A custom picker view for iOS. |
| 首页 | [https://github.com/agiapp/BRPickerView](https://github.com/agiapp/BRPickerView) |
| 许可证 | `MIT / LICENSE` |
| 作者 | `任波 / developer@irenb.com` |
| podspec | `JobsByPods/ManualByOCPods@Pods/BRPickerView/BRPickerView.podspec` |
| source | `{ :git => 'https://github.com/agiapp/BRPickerView.git', :tag => s.version.to_s }` |

## 二、适用场景 <a href="#前言" style="font-size:17px; color:green;"><b>🔼</b></a> <a href="#🔚" style="font-size:17px; color:green;"><b>🔽</b></a>

- 作为外源代码的本地化托管版本，供 `JobsByPods` 内其它 Pod 或 App 通过 [**CocoaPods**](https://cocoapods.org/) 稳定依赖。
- 当 `BRPickerView` 的 `Core`、`Support`、资源、依赖或公开头文件发生变化时，同步更新本 README，避免后续排查只看源码不看边界。
...
```text
BRPickerView/
├── BRPickerView.podspec  # Pod 描述文件
├── README.md  # 当前自述
├── BRPickerView.h  # 根入口头文件
...
- `BRPickerView.podspec` 是当前 Pod 的 [**CocoaPods**](https://cocoapods.org/) 描述入口。
...
- `Core` 当前包含 15 个文件，其中源码 / 头文件 15 个；按 Jobs 规范，它是 `BRPickerView` 对外公开 API 和核心实现的边界。
...
- 如果 `BRPickerView.h` 不是最终公开入口，请先修正 `BRPickerView.podspec` 的 `public_header_files` 和入口头设计，再修改调用方。
...
修改 `BRPickerView` 后，优先按风险从低到高验证：

```shell
ruby -c BRPickerView.podspec
```

```shell
pod lib lint BRPickerView.podspec --allow-warnings --verbose
```
