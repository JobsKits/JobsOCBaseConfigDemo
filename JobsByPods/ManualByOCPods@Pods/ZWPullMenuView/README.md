...
| 项目 | 内容 |
| ---- | ---- |
| Pod 名称 | `ZWPullMenuView` |
| Pod 类型 | 手动托管第三方 Pod |
| 版本 | `0.1.0` |
| 平台 | `ios 12.0` |
| 摘要 | Pull menu view for iOS. |
| 首页 | [https://github.com/Jobs/ZWPullMenuView](https://github.com/Jobs/ZWPullMenuView) |
| 许可证 | `MIT / LICENSE` |
...
- 当 `ZWPullMenuView` 的 `Core`、`Support`、资源、依赖或公开头文件发生变化时，同步更新本 README，避免后续排查只看源码不看边界。
...
```text
ZWPullMenuView/
├── ZWPullMenuView.podspec  # Pod 描述文件
├── README.md  # 当前自述
├── Core/  # 公开 API 与核心实现，8 个文件
└── LICENSE  # 许可证文件
```

- `ZWPullMenuView.podspec` 是当前 Pod 的 [**CocoaPods**](https://cocoapods.org/) 描述入口。
...
- `Core` 当前包含 8 个文件，其中源码 / 头文件 8 个；按 Jobs 规范，它是 `ZWPullMenuView` 对外公开 API 和核心实现的边界。
...
```objc
#if __has_include(<ZWPullMenuView/ZWPullMenuCell.h>)
...
- 如果 `ZWPullMenuCell.h` 不是最终公开入口，请先修正 `ZWPullMenuView.podspec` 的 `public_header_files` 和入口头设计，再修改调用方。
...
修改 `ZWPullMenuView` 后，优先按风险从低到高验证：

```shell
ruby -c ZWPullMenuView.podspec
```

```shell
pod lib lint ZWPullMenuView.podspec --allow-warnings --verbose
```
