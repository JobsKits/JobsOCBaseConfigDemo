...
> 这份自述用于记录 `TFPopup` 在 Jobs 本地 [**CocoaPods**](https://cocoapods.org/) 体系里的职责边界、目录结构、依赖关系和验证方式。
补充描述：TFPopup is a powerful pop-up tool for iOS.
...
| 版本 | `1.4.4` |
| 平台 | `ios 12.0` |
| 摘要 | A powerful pop-up tool |
| 首页 | [https://github.com/shmxybfq/TFPopup](https://github.com/shmxybfq/TFPopup) |
| 许可证 | `MIT / LICENSE` |
| 作者 | `ZTF / 927141965@qq.com` |
...
- 当 `TFPopup` 的 `Core`、`Support`、资源、依赖或公开头文件发生变化时，同步更新本 README，避免后续排查只看源码不看边界。
...
- `TFPopup.podspec` 是当前 Pod 的 [**CocoaPods**](https://cocoapods.org/) 描述入口。
...
- `Core` 当前包含 10 个文件，其中源码 / 头文件 9 个；按 Jobs 规范，它是 `TFPopup` 对外公开 API 和核心实现的边界。
...
- 如果 `TFPopup.h` 不是最终公开入口，请先修正 `TFPopup.podspec` 的 `public_header_files` 和入口头设计，再修改调用方。
...
修改 `TFPopup` 后，优先按风险从低到高验证：

```shell
ruby -c TFPopup.podspec
```

```shell
pod lib lint TFPopup.podspec --allow-warnings --verbose
```
