...
> 这份自述用于记录 `GTCaptcha4` 在 Jobs 本地 [**CocoaPods**](https://cocoapods.org/) 体系里的职责边界、目录结构、依赖关系和验证方式。
...
| 版本 | `1.8.8` |
| 平台 | `ios 12.0` |
| 摘要 | GTCaptcha4 wrapper |
| 首页 | [https://git.tigerbrokers.net/ios/GTCaptcha4](https://git.tigerbrokers.net/ios/GTCaptcha4) |
| 许可证 | `MIT / LICENSE` |
| 作者 | `chenxin / chenxin@tigerbrokers.com` |
...
- 当 `GTCaptcha4` 的 `Core`、`Support`、资源、依赖或公开头文件发生变化时，同步更新本 README，避免后续排查只看源码不看边界。
...
```text
GTCaptcha4/
├── GTCaptcha4.podspec  # Pod 描述文件
├── README.md  # 当前自述
└── LICENSE  # 许可证文件
```

- `GTCaptcha4.podspec` 是当前 Pod 的 [**CocoaPods**](https://cocoapods.org/) 描述入口。
...
- podspec 未显式声明 `public_header_files`，请以 `source_files` 和实际头文件为准。

### 5.2、源码入口

- podspec 未显式声明 `source_files`，请直接检查 podspec。
...
- 如果 `GTCaptcha4.h` 不是最终公开入口，请先修正 `GTCaptcha4.podspec` 的 `public_header_files` 和入口头设计，再修改调用方。

## 七、资源说明 <a href="#前言" style="font-size:17px; color:green;"><b>🔼</b></a> <a href="#🔚" style="font-size:17px; color:green;"><b>🔽</b></a>

- 当前目录扫描到资源类文件 26 个，`Resources` 目录文件 0 个。
- podspec 资源声明如下：
...
修改 `GTCaptcha4` 后，优先按风险从低到高验证：

```shell
ruby -c GTCaptcha4.podspec
```

```shell
pod lib lint GTCaptcha4.podspec --allow-warnings --verbose
```
