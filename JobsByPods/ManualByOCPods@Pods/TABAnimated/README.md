...
> 这份自述用于记录 `TABAnimated` 在 Jobs 本地 [**CocoaPods**](https://cocoapods.org/) 体系里的职责边界、目录结构、依赖关系和验证方式。
补充描述：TABAnimated是一个ios平台上的网络过渡动画(骨架屏)的封装，目前仅支持oc
...
| 版本 | `2.6.6` |
| 平台 | `ios 12.0` |
| 摘要 | TABAnimated是一个ios平台上的网络过渡动画(骨架屏)的封装 |
| 首页 | [https://github.com/tigerAndBull/TABAnimated](https://github.com/tigerAndBull/TABAnimated) |
| 许可证 | `MIT / LICENSE` |
| 作者 | `tigerAndBull / 1429299849@qq.com` |
| podspec | `JobsByPods/ManualByOCPods@Pods/TABAnimated/TABAnimated.podspec` |
| source | `{ :git => "https://github.com/tigerAndBull/TABAnimated.git", :tag => spec.version.to_s }` |

## 二、适用场景 <a href="#前言" style="font-size:17px; color:green;"><b>🔼</b></a> <a href="#🔚" style="font-size:17px; color:green;"><b>🔽</b></a>

- 作为外源代码的本地化托管版本，供 `JobsByPods` 内其它 Pod 或 App 通过 [**CocoaPods**](https://cocoapods.org/) 稳定依赖。
- 当 `TABAnimated` 的 `Core`、`Support`、资源、依赖或公开头文件发生变化时，同步更新本 README，避免后续排查只看源码不看边界。
...
```text
TABAnimated/
├── TABAnimated.podspec  # Pod 描述文件
├── README.md  # 当前自述
└── LICENSE  # 许可证文件
```

- `TABAnimated.podspec` 是当前 Pod 的 [**CocoaPods**](https://cocoapods.org/) 描述入口。
...
- 如果 `TABAnimated.h` 不是最终公开入口，请先修正 `TABAnimated.podspec` 的 `public_header_files` 和入口头设计，再修改调用方。

## 七、资源说明 <a href="#前言" style="font-size:17px; color:green;"><b>🔼</b></a> <a href="#🔚" style="font-size:17px; color:green;"><b>🔽</b></a>

- 当前目录扫描到资源类文件 42 个，`Resources` 目录文件 0 个。
- podspec 资源声明如下：
...
修改 `TABAnimated` 后，优先按风险从低到高验证：

```shell
ruby -c TABAnimated.podspec
```

```shell
pod lib lint TABAnimated.podspec --allow-warnings --verbose
```
