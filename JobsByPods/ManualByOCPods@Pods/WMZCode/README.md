...
> 这份自述用于记录 `WMZCode` 在 Jobs 本地 [**CocoaPods**](https://cocoapods.org/) 体系里的职责边界、目录结构、依赖关系和验证方式。
补充描述：支持多种验证码/滑块验证视图。
...
| 平台 | `ios 12.0` |
| 摘要 | 滑块验证 |
| 首页 | [https://github.com/wwmz/WMZCode](https://github.com/wwmz/WMZCode) |
| 许可证 | `MIT / LICENSE` |
| 作者 | `wmz / 925457662@qq.com` |
...
- 当 `WMZCode` 的 `Core`、`Support`、资源、依赖或公开头文件发生变化时，同步更新本 README，避免后续排查只看源码不看边界。
...
```text
WMZCode/
├── WMZCode.podspec  # Pod 描述文件
├── README.md  # 当前自述
└── LICENSE  # 许可证文件
```

- `WMZCode.podspec` 是当前 Pod 的 [**CocoaPods**](https://cocoapods.org/) 描述入口。
...
- `WMZCodeViewDefine.h`
- `WMZBannerView/WMZCodeView.h`

### 5.2、源码入口

- `WMZCodeViewDefine.h`
- `WMZBannerView/WMZCodeView.{h,m}`
...
```objc
#if __has_include(<WMZCode/WMZCodeViewDefine.h>)
...
- 如果 `WMZCodeViewDefine.h` 不是最终公开入口，请先修正 `WMZCode.podspec` 的 `public_header_files` 和入口头设计，再修改调用方。
...
修改 `WMZCode` 后，优先按风险从低到高验证：

```shell
ruby -c WMZCode.podspec
```

```shell
pod lib lint WMZCode.podspec --allow-warnings --verbose
```
