...
static inline void FSCalendarAssertDateInBounds(NSDate *date,
                                                NSCalendar *calendar,
                                                NSDate *minimumDate,
                                                NSDate *maximumDate) {
    BOOL valid = YES;
...
Prop_strong()UIView *daysContainer;
Prop_strong()FSCalendarCollectionView   *collectionView;
Prop_strong()FSCalendarCollectionViewLayout *collectionViewLayout;
Prop_strong()FSCalendarTransitionCoordinator *transitionCoordinator;
Prop_strong()FSCalendarCalculator       *calculator;
Prop_strong()FSCalendarHeaderTouchDeliver *deliver;
...
Prop_strong()NSMutableArray <NSOperation *>*didLayoutOperations;
Prop_assign(readonly)BOOL floatingMode;
Prop_assign(readonly)BOOL hasValidateVisibleLayout;
Prop(readonly)NSArray *visibleStickyHeaders;
Prop(readonly)FSCalendarOrientation currentCalendarOrientation;
Prop_strong()FSCalendarDelegationProxy *dataSourceProxy;
Prop_strong()FSCalendarDelegationProxy *delegateProxy;
Prop_strong()NSIndexPath *lastPressedIndexPath;
Prop_strong()NSMapTable *visibleSectionHeaders;
...
-(UIView *)daysContainer{
    if(!_daysContainer){
        @jobs_weakify(self)
        _daysContainer = jobsMakeView(^(__kindof UIView * _Nullable view) {
            @jobs_strongify(self)
            view.backgroundColor = UIColor.clearColor;
            view.clipsToBounds = YES;
...
-(__kindof UICollectionView *)collectionView{
    if(!_collectionView){
        _collectionView = [FSCalendarCollectionView.alloc initWithFrame:CGRectZero
                                                   collectionViewLayout:self.collectionViewLayout];
        _collectionView.internalDelegate = self;
        ...
        [_collectionView registerClass:FSCalendarCell.class forCellWithReuseIdentifier:FSCalendarDefaultCellReuseIdentifier];
        [_collectionView registerClass:FSCalendarBlankCell.class forCellWithReuseIdentifier:FSCalendarBlankCellReuseIdentifier];
        [_collectionView registerClass:FSCalendarStickyHeader.class forSupplementaryViewOfKind:UICollectionElementKindSectionHeader
