...
| 版本 | `2.8.4` |
| 平台 | `ios 12.0` |
| 摘要 | A superior calendar for iOS with excellent performance. |
| 首页 | [https://github.com/WenchaoD/FSCalendar](https://github.com/WenchaoD/FSCalendar) |
| 许可证 | `MIT / LICENSE` |
| 作者 | `Wenchao Ding / wenchao.d@gmail.com` |
...
- 当 `FSCalendar` 的 `Core`、`Support`、资源、依赖或公开头文件发生变化时，同步更新本 README，避免后续排查只看源码不看边界。
...
```text
FSCalendar/
├── FSCalendar.podspec  # Pod 描述文件
├── README.md  # 当前自述
├── Core/  # 公开 API 与核心实现，30 个文件
└── LICENSE  # 许可证文件
```

- `FSCalendar.podspec` 是当前 Pod 的 [**CocoaPods**](https://cocoapods.org/) 描述入口。
...
- `Core` 当前包含 30 个文件，其中源码 / 头文件 30 个；按 Jobs 规范，它是 `FSCalendar` 对外公开 API 和核心实现的边界。
...
- 如果 `FSCalendarDynamicHeader.h` 不是最终公开入口，请先修正 `FSCalendar.podspec` 的 `public_header_files` 和入口头设计，再修改调用方。
...
修改 `FSCalendar` 后，优先按风险从低到高验证：

```shell
ruby -c FSCalendar.podspec
```

```shell
pod lib lint FSCalendar.podspec --allow-warnings --verbose
```
