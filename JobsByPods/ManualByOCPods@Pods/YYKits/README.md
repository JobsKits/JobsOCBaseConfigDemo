...
> 这份自述用于记录 `YYKits` 在 Jobs 本地 [**CocoaPods**](https://cocoapods.org/) 体系里的职责边界、目录结构、依赖关系和验证方式。
补充描述：YYKits is a lightweight local Objective-C pod that packages YYFPSLabel and YYWeakProxy for convenient integration through CocoaPods.
...
| 版本 | `0.1.0` |
| 平台 | `ios 12.0` |
| 摘要 | Local Objective-C pod that provides YYFPSLabel and YYWeakProxy. |
| 首页 | [https://github.com/Jobs/YYKits](https://github.com/Jobs/YYKits) |
| 许可证 | `MIT / LICENSE` |
...
- 当 `YYKits` 的 `Core`、`Support`、资源、依赖或公开头文件发生变化时，同步更新本 README，避免后续排查只看源码不看边界。
...
```text
YYKits/
├── YYKits.podspec  # Pod 描述文件
├── README.md  # 当前自述
├── Core/  # 公开 API 与核心实现，4 个文件
└── LICENSE  # 许可证文件
```

- `YYKits.podspec` 是当前 Pod 的 [**CocoaPods**](https://cocoapods.org/) 描述入口。
...
- `Core` 当前包含 4 个文件，其中源码 / 头文件 4 个；按 Jobs 规范，它是 `YYKits` 对外公开 API 和核心实现的边界。
...
- `Core/YYFPSLabel/YYFPSLabel.h`
- `Core/YYWeakProxy/YYWeakProxy.h`

### 5.2、源码入口

- `Core/YYFPSLabel/YYFPSLabel.{h,m}`
- `Core/YYWeakProxy/YYWeakProxy.{h,m}`
...
```objc
#if __has_include(<YYKits/YYFPSLabel.h>)
#import <YYKits/YYFPSLabel.h>
#else
#import "YYFPSLabel.h"
#endif
```
...
- 如果 `YYFPSLabel.h` 不是最终公开入口，请先修正 `YYKits.podspec` 的 `public_header_files` 和入口头设计，再修改调用方。
...
修改 `YYKits` 后，优先按风险从低到高验证：

```shell
ruby -c YYKits.podspec
```

```shell
pod lib lint YYKits.podspec --allow-warnings --verbose
```
