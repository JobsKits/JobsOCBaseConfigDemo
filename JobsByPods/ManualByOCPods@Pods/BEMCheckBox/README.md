...
> 这份自述用于记录 `BEMCheckBox` 在 Jobs 本地 [**CocoaPods**](https://cocoapods.org/) 体系里的职责边界、目录结构、依赖关系和验证方式。
补充描述：BEMCheckBox is an open source checkbox UIControl for iOS, written in Objective-C.
...
| 版本 | `1.4.1` |
| 平台 | `ios 9.0` |
| 摘要 | Tastefully crafted Checkbox for iOS. |
| 首页 | [https://github.com/Boris-Em/BEMCheckBox](https://github.com/Boris-Em/BEMCheckBox) |
| 许可证 | `MIT / LICENSE` |
| 作者 | `Boris Emorine / boris.emorine@gmail.com` |
| podspec | `JobsByPods/ManualByOCPods@Pods/BEMCheckBox/BEMCheckBox.podspec` |
| source | `{ :git => 'https://github.com/Boris-Em/BEMCheckBox.git', :tag => spec.version.to_s }` |

## 二、适用场景 <a href="#前言" style="font-size:17px; color:green;"><b>🔼</b></a> <a href="#🔚" style="font-size:17px; color:green;"><b>🔽</b></a>

- 作为外源代码的本地化托管版本，供 `JobsByPods` 内其它 Pod 或 App 通过 [**CocoaPods**](https://cocoapods.org/) 稳定依赖。
- 当 `BEMCheckBox` 的 `Core`、`Support`、资源、依赖或公开头文件发生变化时，同步更新本 README，避免后续排查只看源码不看边界。
...
- `BEMCheckBox.podspec` 是当前 Pod 的 [**CocoaPods**](https://cocoapods.org/) 描述入口。
...
- `Core` 当前包含 8 个文件，其中源码 / 头文件 8 个；按 Jobs 规范，它是 `BEMCheckBox` 对外公开 API 和核心实现的边界。
...
修改 `BEMCheckBox` 后，优先按风险从低到高验证：

```shell
ruby -c BEMCheckBox.podspec
```

```shell
pod lib lint BEMCheckBox.podspec --allow-warnings --verbose
```
