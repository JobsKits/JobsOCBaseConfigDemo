...
> 这份自述用于记录 `DeviceKit` 在 Jobs 本地 [**CocoaPods**](https://cocoapods.org/) 体系里的职责边界、目录结构、依赖关系和验证方式。
补充描述：DeviceKit is a local Objective-C device information library. It provides device type mapping, battery state, disk space and Apple Pencil support helpers.
...
| 版本 | `1.0.0` |
| 平台 | `未在 podspec 中声明` |
| 摘要 | Objective-C port scaffold of DeviceKit. |
| 首页 | [LOCAL](LOCAL) |
| 许可证 | `MIT / LICENSE` |
| 作者 | `Jobs / local` |
...
- 当 `DeviceKit` 的 `Core`、`Support`、资源、依赖或公开头文件发生变化时，同步更新本 README，避免后续排查只看源码不看边界。
...
```text
DeviceKit/
├── DeviceKit.podspec  # Pod 描述文件
├── DeviceKitObjc.h  # 根聚合头文件
├── README.md  # 当前自述
├── Core/  # 公开 API 与核心实现，12 个文件
├── Resource/  # 隐私清单
└── LICENSE  # 许可证文件
```

- `DeviceKit.podspec` 是当前 Pod 的 [**CocoaPods**](https://cocoapods.org/) 描述入口。
...
- `Core` 当前包含 12 个文件，其中源码 / 头文件 12 个；按 Jobs 规范，它是 `DeviceKit` 对外公开 API 和核心实现的边界。
...
```objc
#if __has_include(<DeviceKit/DeviceKitObjc.h>)
#import <DeviceKit/DeviceKitObjc.h>
#else
#import "DeviceKitObjc.h"
#endif
```

- 自建 Pod 对外优先引用公共入口头，不要绕开聚合头直接引用 `Support` 内部子头。
- 如果 `DeviceKitObjc.h` 不是最终公开入口，请先修正 `DeviceKit.podspec` 的 `public_header_files` 和入口头设计，再修改调用方。

## 七、资源说明 <a href="#前言" style="font-size:17px; color:green;"><b>🔼</b></a> <a href="#🔚" style="font-size:17px; color:green;"><b>🔽</b></a>

- `Resource/PrivacyInfo.xcprivacy` 由 `DeviceKitResources` 资源包声明，不放在 Pod 根目录或 `Core`。

## 八、验证方式 <a href="#前言" style="font-size:17px; color:green;"><b>🔼</b></a> <a href="#🔚" style="font-size:17px; color:green;"><b>🔽</b></a>

修改 `DeviceKit` 后，优先按风险从低到高验证：

```shell
ruby -c DeviceKit.podspec
```

```shell
pod lib lint DeviceKit.podspec --allow-warnings --verbose
```
