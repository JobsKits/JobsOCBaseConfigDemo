...
Prop_strong(class,readonly)TKPermissionHealth *shared;
...
+(TKPermissionHealth *)shared{
    if (!_shared) {
        _shared = TKPermissionHealth.new;
...
+(void)authWithAlert:(BOOL)isAlert
    ...
    obj.isAlert = isAlert;
    if ([HKHealthStore isHealthDataAvailable]) {
        [obj.healthStore requestAuthorizationToShareTypes:self.shared.writeTypes 
                                                readTypes:self.shared.readTypes
