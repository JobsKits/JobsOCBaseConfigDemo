...
| 项目 | 内容 |
| ---- | ---- |
| Pod 名称 | `TKPermissionKit` |
| Pod 类型 | 手动托管第三方 Pod |
| 版本 | `1.2.9` |
| 平台 | `ios 12.0` |
| 摘要 | iOS权限管理工具 |
| 首页 | [https://github.com/RANSAA/TKPermissionKit](https://github.com/RANSAA/TKPermissionKit) |
| 许可证 | `MIT / LICENSE` |
| 作者 | `sayaDev / 1352892108@qq.com` |
...
- 当 `TKPermissionKit` 的 `Core`、`Support`、资源、依赖或公开头文件发生变化时，同步更新本 README，避免后续排查只看源码不看边界。
...
```text
TKPermissionKit/
├── TKPermissionKit.podspec  # Pod 描述文件
├── README.md  # 当前自述
├── TKPermissionKit.h  # 根入口头文件
└── LICENSE  # 许可证文件
```

- `TKPermissionKit.podspec` 是当前 Pod 的 [**CocoaPods**](https://cocoapods.org/) 描述入口。
...
- 如果 `TKPermissionKit.h` 不是最终公开入口，请先修正 `TKPermissionKit.podspec` 的 `public_header_files` 和入口头设计，再修改调用方。

## 七、资源说明 <a href="#前言" style="font-size:17px; color:green;"><b>🔼</b></a> <a href="#🔚" style="font-size:17px; color:green;"><b>🔽</b></a>

- 当前目录扫描到资源类文件 6 个，`Resources` 目录文件 0 个。
- podspec 资源声明如下：
...
修改 `TKPermissionKit` 后，优先按风险从低到高验证：

```shell
ruby -c TKPermissionKit.podspec
```

```shell
pod lib lint TKPermissionKit.podspec --allow-warnings --verbose
```
