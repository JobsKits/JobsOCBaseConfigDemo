...
> 这份自述用于记录 `XYColorOC` 在 Jobs 本地 [**CocoaPods**](https://cocoapods.org/) 体系里的职责边界、目录结构、依赖关系和验证方式。
补充描述：Objective-C only fork of XYColorOC. It keeps the umbrella header XYColorOC.h so consumers can import <XYColorOC/XYColorOC.h>.
...
| 版本 | `1.1.2` |
| 平台 | `未在 podspec 中声明` |
| 摘要 | An Objective-C only dark mode color helper for UIKit and CALayer. |
| 首页 | [https://github.com/RayJiang16/XYColor](https://github.com/RayJiang16/XYColor) |
| 许可证 | `MIT / LICENSE` |
| 作者 | `RayJiang / 1184731421@qq.com` |
| podspec | `JobsByPods/ManualByOCPods@Pods/XYColorOC/XYColorOC.podspec` |
| source | `{ :git => 'https://github.com/RayJiang16/XYColor.git', :tag => spec.version.to_s }` |

## 二、适用场景 <a href="#前言" style="font-size:17px; color:green;"><b>🔼</b></a> <a href="#🔚" style="font-size:17px; color:green;"><b>🔽</b></a>

- 作为外源代码的本地化托管版本，供 `JobsByPods` 内其它 Pod 或 App 通过 [**CocoaPods**](https://cocoapods.org/) 稳定依赖。
- 当 `XYColorOC` 的 `Core`、`Support`、资源、依赖或公开头文件发生变化时，同步更新本 README，避免后续排查只看源码不看边界。
...
- `XYColorOC.podspec` 是当前 Pod 的 [**CocoaPods**](https://cocoapods.org/) 描述入口。
...
- 如果 `XYColorOC.h` 不是最终公开入口，请先修正 `XYColorOC.podspec` 的 `public_header_files` 和入口头设计，再修改调用方。
...
修改 `XYColorOC` 后，优先按风险从低到高验证：

```shell
ruby -c XYColorOC.podspec
```

```shell
pod lib lint XYColorOC.podspec --allow-warnings --verbose
```
