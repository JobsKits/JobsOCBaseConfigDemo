//
//  DatasBundleByUIKit.h
//  JobsModel
//
//  Created by Jobs on 2026年5月13日，星期三.
//

#ifndef DatasBundleByUIKit_h
#define DatasBundleByUIKit_h

#import <JobsModel/JobsGestureModel.h> // 手势配置数据源
#import <JobsModel/JobsTimeModel.h>    // 时间/日期/日历的处理
#import <JobsModel/URLManagerModel.h>
#import <JobsModel/JobsKeyValueModel.h>
#import <JobsModel/NSNotificationKeyboardModel.h>
#import <JobsModel/JobsAlertModel.h>
#import <JobsModel/UserDefaultModel.h>
#import <JobsModel/JobsCorModel.h>
#import <JobsModel/BaseModel.h>
#import <JobsModel/JobsImageModel.h>
#import <JobsModel/UNNotificationRequestModel.h>
#import <JobsModel/NotificationModel.h>
#import <JobsModel/JobsLocationModel.h>
#import <JobsModel/UIViewModel.h>      // 本类是自定义类，但是是对系统类的仿写，目的是承接上下文数据 成一束，方便管理
#import <JobsModel/UIButtonModel.h>    // 按钮数据源
#import <JobsModel/UITextModel.h>      // 文本数据源
#import <JobsModel/UITextFieldModel.h> // 输入框数据源
#import <JobsModel/JobsHeaderFooterViewModel.h>
#import <JobsModel/FileFolderHandleModel.h>
#import <JobsModel/JobsNetworkSource.h>
#import <JobsModel/JobsWalletCardModel.h>

#endif /* DatasBundleByUIKit_h */
