...
#import <JobsCalendar/JobsCalendar.h>

@interface JobsCalendar ()

Prop_strong(readwrite)JobsCalendarAppearance *appearance;
...
Prop_strong()NSMutableArray<JobsCalendarDayCell *> *dayCells;
...
-(void)jobsCellClickEvent:(JobsCalendarDayCell *)cell;
...
-(JobsCalendarMonthPosition)jobsMonthPositionByDate:(NSDate *)date;
-(void)jobsNotifyBoundingRectIfNeeded:(BOOL)animated;

@end

@implementation JobsCalendar
...
-(void)layoutSubviews{
    ...
    NSInteger rowCount = self.scope == JobsCalendarScopeWeek ? 1 : 6;
    ...
    [self.dayCells enumerateObjectsUsingBlock:^(JobsCalendarDayCell *cell, NSUInteger idx, BOOL *stop) {
        NSInteger row = idx / 7;
...
-(void)setScope:(JobsCalendarScope)scope{
...
-(void)setPlaceholderType:(JobsCalendarPlaceholderType)placeholderType{
...
-(CGRect)boundingRectForScope:(JobsCalendarScope)scope{
    NSInteger rowCount = scope == JobsCalendarScopeWeek ? 1 : 6;
...
-(void)jobsCommonInit{
    self.appearance = JobsCalendarAppearance.new;
    ...
    self.scope = JobsCalendarScopeMonth;
    self.placeholderType = JobsCalendarPlaceholderTypeFillSixRows;
...
-(void)jobsInstallSubviews{
    ...
    for (NSInteger index = 0; index < 42; index++) {
        JobsCalendarDayCell *cell = [JobsCalendarDayCell.alloc initWithFrame:CGRectZero];
...
-(void)jobsReloadHeader{
    ...
    if (self.appearance.caseOptions & JobsCalendarCaseOptionsHeaderUsesUpperCase) title = title.uppercaseString;
...
-(void)jobsReloadWeekdayLabels{
    ...
    for (NSInteger index = 0; index < 7; index++) {
        ...
        if (self.appearance.caseOptions & JobsCalendarCaseOptionsWeekdayUsesSingleUpperCase) {
            symbol = symbol.length ? [symbol substringToIndex:1].uppercaseString : symbol;
        }else if (self.appearance.caseOptions & JobsCalendarCaseOptionsWeekdayUsesUpperCase) {
...
-(void)jobsReloadDayCells{
    ...
    [self.dayCells enumerateObjectsUsingBlock:^(JobsCalendarDayCell *cell, NSUInteger idx, BOOL *stop) {
        ...
        JobsCalendarMonthPosition position = [self jobsMonthPositionByDate:date];
        BOOL placeholder = position != JobsCalendarMonthPositionCurrent;
        BOOL hiddenByPlaceholder = self.placeholderType == JobsCalendarPlaceholderTypeNone && placeholder;
        NSString *title = nil;
...
-(void)jobsCellClickEvent:(JobsCalendarDayCell *)cell{
    ...
    if (position != JobsCalendarMonthPositionCurrent) [self setCurrentPage:date animated:YES];
...
-(JobsCalendarMonthPosition)jobsMonthPositionByDate:(NSDate *)date{
    if ([self jobsDate:date isSameMonthAsDate:self.currentPage]) return JobsCalendarMonthPositionCurrent;
    return [date compare:self.currentPage] == NSOrderedAscending ? JobsCalendarMonthPositionPrevious : JobsCalendarMonthPositionNext;
}
