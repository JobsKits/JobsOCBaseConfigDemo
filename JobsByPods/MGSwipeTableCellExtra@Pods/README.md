...
> 这份自述用于记录 `MGSwipeTableCellExtra` 在 Jobs 本地 [**CocoaPods**](https://cocoapods.org/) 体系里的职责边界、目录结构、依赖关系和验证方式。

## 一、Pod 定位 <a href="#前言" style="font-size:17px; color:green;"><b>🔼</b></a> <a href="#🔚" style="font-size:17px; color:green;"><b>🔽</b></a>

| 项目 | 内容 |
| ---- | ---- |
| Pod 名称 | `MGSwipeTableCellExtra` |
| Pod 类型 | 本地扩展 Pod |
| 版本 | `1.0.0` |
| 平台 | `ios 12.0` |
| 摘要 | MGSwipeTableCell helpers for Jobs. |
| 首页 | [https://example.local/MGSwipeTableCellExtra](https://example.local/MGSwipeTableCellExtra) |
| 许可证 | `MIT / LICENSE` |
...
- 为 `MGSwipeTableCell` 或同名上游能力提供 Jobs 项目内的分类、桥接、资源或边界适配。
- 当 `MGSwipeTableCellExtra` 的 `Core`、`Support`、资源、依赖或公开头文件发生变化时，同步更新本 README，避免后续排查只看源码不看边界。
- 参与本地 Pods 拆分时，先确认能力归属，再决定放入当前 Pod、迁移到 `Support`，还是下沉为更基础的公共 Pod。

## 三、目录结构 <a href="#前言" style="font-size:17px; color:green;"><b>🔼</b></a> <a href="#🔚" style="font-size:17px; color:green;"><b>🔽</b></a>

```text
MGSwipeTableCellExtra@Pods/
├── MGSwipeTableCellExtra.podspec  # Pod 描述文件
├── README.md  # 当前自述
├── MGSwipeTableCellExtra.h  # 根入口头文件
├── Core/  # 公开 API 与核心实现，3 个文件
└── LICENSE  # 许可证文件
```

- `MGSwipeTableCellExtra.podspec` 是当前 Pod 的 [**CocoaPods**](https://cocoapods.org/) 描述入口。
...
- `Core` 当前包含 3 个文件，其中源码 / 头文件 3 个；按 Jobs 规范，它是 `MGSwipeTableCellExtra` 对外公开 API 和核心实现的边界。
...
- `MGSwipeTableCell+Extra` 实现 `byDelegate(...)`，用于链式设置 `MGSwipeTableCellDelegate`。
- `MGSwipeTableCell+Extra` 实现 `byAllowsMultipleSwipe(...)`，用于链式设置 `allowsMultipleSwipe`，避免调用方只声明协议但运行时找不到方法。
...
- 如果 `MGSwipeTableCellExtra.h` 不是最终公开入口，请先修正 `MGSwipeTableCellExtra.podspec` 的 `public_header_files` 和入口头设计，再修改调用方。
...
修改 `MGSwipeTableCellExtra` 后，优先按风险从低到高验证：

```shell
ruby -c MGSwipeTableCellExtra.podspec
```

```shell
pod lib lint MGSwipeTableCellExtra.podspec --allow-warnings --verbose
```
