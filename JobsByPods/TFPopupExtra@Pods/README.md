...
> 这份自述用于记录 `TFPopupExtra` 在 Jobs 本地 [**CocoaPods**](https://cocoapods.org/) 体系里的职责边界、目录结构、依赖关系和验证方式。
补充描述：TFPopupExtra is a local Objective-C category library providing TFPopup helper methods for popup views, toast presentation, filtration views, and custom views.
...
| 版本 | `1.0.0` |
| 平台 | `ios 12.0` |
| 摘要 | TFPopup category extensions for Jobs projects. |
| 首页 | [https://example.local/TFPopupExtra](https://example.local/TFPopupExtra) |
| 许可证 | `MIT / LICENSE` |
...
- 为 `TFPopup` 或同名上游能力提供 Jobs 项目内的分类、桥接、资源或边界适配。
- 当 `TFPopupExtra` 的 `Core`、`Support`、资源、依赖或公开头文件发生变化时，同步更新本 README，避免后续排查只看源码不看边界。
...
```text
TFPopupExtra@Pods/
├── TFPopupExtra.podspec  # Pod 描述文件
├── README.md  # 当前自述
├── TFPopupExtra.h  # 根入口头文件
...
- `TFPopupExtra.podspec` 是当前 Pod 的 [**CocoaPods**](https://cocoapods.org/) 描述入口。
...
- `Core` 当前包含 4 个文件，其中源码 / 头文件 4 个；按 Jobs 规范，它是 `TFPopupExtra` 对外公开 API 和核心实现的边界。
- `Support` 当前包含 0 个文件，其中源码 / 头文件 0 个；它只服务当前 Pod 内部实现，不建议被 App 层或其它 Pod 直接引用。
...
- 如果 `TFPopupExtra.h` 不是最终公开入口，请先修正 `TFPopupExtra.podspec` 的 `public_header_files` 和入口头设计，再修改调用方。
...
修改 `TFPopupExtra` 后，优先按风险从低到高验证：

```shell
ruby -c TFPopupExtra.podspec
```

```shell
pod lib lint TFPopupExtra.podspec --allow-warnings --verbose
```
