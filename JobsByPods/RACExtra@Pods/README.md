...
> 这份自述用于记录 `RACExtra` 在 Jobs 本地 [**CocoaPods**](https://cocoapods.org/) 体系里的职责边界、目录结构、依赖关系和验证方式。
...
| 版本 | `1.0.0` |
| 平台 | `ios 12.0` |
| 摘要 | ReactiveCocoa extension macros for Jobs. |
| 首页 | [https://example.local/RACExtra](https://example.local/RACExtra) |
| 许可证 | `MIT / README.md` |
...
- 为 `RAC` 或同名上游能力提供 Jobs 项目内的分类、桥接、资源或边界适配。
- 当 `RACExtra` 的 `Core`、`Support`、资源、依赖或公开头文件发生变化时，同步更新本 README，避免后续排查只看源码不看边界。
...
```text
RACExtra@Pods/
├── RACExtra.podspec  # Pod 描述文件
├── README.md  # 当前自述
├── RACExtra.h  # 根入口头文件
└── Core/  # 公开 API 与核心实现，5 个文件
```

- `RACExtra.podspec` 是当前 Pod 的 [**CocoaPods**](https://cocoapods.org/) 描述入口。
...
- `Core` 当前包含 5 个文件，其中源码 / 头文件 5 个；按 Jobs 规范，它是 `RACExtra` 对外公开 API 和核心实现的边界。
...
```objc
#if __has_include(<RACExtra/RACExtra.h>)
#import <RACExtra/RACExtra.h>
#else
#import "RACExtra.h"
#endif
```
...
- 如果 `RACExtra.h` 不是最终公开入口，请先修正 `RACExtra.podspec` 的 `public_header_files` 和入口头设计，再修改调用方。
...
修改 `RACExtra` 后，优先按风险从低到高验证：

```shell
ruby -c RACExtra.podspec
```

```shell
pod lib lint RACExtra.podspec --allow-warnings --verbose
```
