...
| 版本 | `0.0.1` |
| 平台 | `ios 12.0` |
| 摘要 | API request layer built on top of YTKNetwork. |
| 首页 | [https://github.com/yourname/JobsAPIs](https://github.com/yourname/JobsAPIs) |
| 许可证 | `MIT / LICENSE` |
| 作者 | `Jobs / jobs@example.com` |
| podspec | `JobsByPods/JobsAPIs@Pods/JobsAPIs.podspec` |
| source | `{ :git => 'https://github.com/yourname/JobsAPIs.git', :tag => spec.version.to_s }` |

## 二、适用场景 <a href="#前言" style="font-size:17px; color:green;"><b>🔼</b></a> <a href="#🔚" style="font-size:17px; color:green;"><b>🔽</b></a>

- 作为 Jobs 项目内的独立能力 Pod，向 App 或其它 Pod 提供 `JobsAPIs` 相关能力。
- 当 `JobsAPIs` 的 `Core`、`Support`、资源、依赖或公开头文件发生变化时，同步更新本 README，避免后续排查只看源码不看边界。
...
```text
JobsAPIs@Pods/
├── JobsAPIs.podspec  # Pod 描述文件
├── README.md  # 当前自述
├── JobsAPIs.h  # 根入口头文件
├── JobsPodspecKit.rb  # 本地 podspec 基座
├── Core/  # 公开 API 与核心实现，34 个文件
├── Support/  # 内部支撑层，36 个文件
└── LICENSE  # 许可证文件
```

- `JobsAPIs.podspec` 是当前 Pod 的 [**CocoaPods**](https://cocoapods.org/) 描述入口。
...
- `Core` 当前包含 34 个文件，其中源码 / 头文件 34 个；按 Jobs 规范，它是 `JobsAPIs` 对外公开 API 和核心实现的边界。
- `Support` 当前包含 36 个文件，其中源码 / 头文件 36 个；它只服务当前 Pod 内部实现，不建议被 App 层或其它 Pod 直接引用。
...
- 如果 `JobsAPIs.h` 不是最终公开入口，请先修正 `JobsAPIs.podspec` 的 `public_header_files` 和入口头设计，再修改调用方。
...
修改 `JobsAPIs` 后，优先按风险从低到高验证：

```shell
ruby -c JobsAPIs.podspec
```

```shell
pod lib lint JobsAPIs.podspec --allow-warnings --verbose
```
