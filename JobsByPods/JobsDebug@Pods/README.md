...
> 这份自述用于记录 `JobsDebug` 在 Jobs 本地 [**CocoaPods**](https://cocoapods.org/) 体系里的职责边界、目录结构、依赖关系和验证方式。
补充描述：JobsDebug is a local Objective-C debug component library that provides debug log description support and debug utilities for development and testing.
...
| 版本 | `1.0.0` |
| 平台 | `ios 12.0` |
| 摘要 | Objective-C debug components for Jobs projects. |
| 首页 | [https://example.local/JobsDebug](https://example.local/JobsDebug) |
| 许可证 | `MIT / LICENSE` |
...
- 作为 Jobs 项目内的独立能力 Pod，向 App 或其它 Pod 提供 `JobsDebug` 相关能力。
- 当 `JobsDebug` 的 `Core`、`Support`、资源、依赖或公开头文件发生变化时，同步更新本 README，避免后续排查只看源码不看边界。
...
- `JobsDebug.podspec` 是当前 Pod 的 [**CocoaPods**](https://cocoapods.org/) 描述入口。
...
- `Core` 当前包含 2 个文件，其中源码 / 头文件 2 个；按 Jobs 规范，它是 `JobsDebug` 对外公开 API 和核心实现的边界。
...
- 如果 `JobsDebug.h` 不是最终公开入口，请先修正 `JobsDebug.podspec` 的 `public_header_files` 和入口头设计，再修改调用方。
...
修改 `JobsDebug` 后，优先按风险从低到高验证：

```shell
ruby -c JobsDebug.podspec
```

```shell
pod lib lint JobsDebug.podspec --allow-warnings --verbose
```
