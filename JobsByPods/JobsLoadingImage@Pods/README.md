...
| 项目 | 内容 |
| ---- | ---- |
| Pod 名称 | `JobsLoadingImage` |
| Pod 类型 | 自建本地 Pod |
| 版本 | `0.0.1` |
| 平台 | `ios 12.0` |
| 摘要 | Bundle image loading helpers extracted into a standalone local pod. |
| 首页 | [https://example.local/JobsLoadingImage](https://example.local/JobsLoadingImage) |
| 许可证 | `MIT / LICENSE` |
...
- 作为 Jobs 项目内的独立能力 Pod，向 App 或其它 Pod 提供 `JobsLoadingImage` 相关能力。
- 当 `JobsLoadingImage` 的 `Core`、`Support`、资源、依赖或公开头文件发生变化时，同步更新本 README，避免后续排查只看源码不看边界。
- 参与本地 Pods 拆分时，先确认能力归属，再决定放入当前 Pod、迁移到 `Support`，还是下沉为更基础的公共 Pod。

## 三、目录结构 <a href="#前言" style="font-size:17px; color:green;"><b>🔼</b></a> <a href="#🔚" style="font-size:17px; color:green;"><b>🔽</b></a>

```text
JobsLoadingImage@Pods/
├── JobsLoadingImage.podspec  # Pod 描述文件
├── README.md  # 当前自述
...
- `JobsLoadingImage.podspec` 是当前 Pod 的 [**CocoaPods**](https://cocoapods.org/) 描述入口。
...
- `Core` 当前包含 2 个文件，其中源码 / 头文件 2 个；按 Jobs 规范，它是 `JobsLoadingImage` 对外公开 API 和核心实现的边界。
...
- 如果 `JobsLoadingImage.h` 不是最终公开入口，请先修正 `JobsLoadingImage.podspec` 的 `public_header_files` 和入口头设计，再修改调用方。
...
修改 `JobsLoadingImage` 后，优先按风险从低到高验证：

```shell
ruby -c JobsLoadingImage.podspec
```

```shell
pod lib lint JobsLoadingImage.podspec --allow-warnings --verbose
```
