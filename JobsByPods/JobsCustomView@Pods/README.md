...
| 项目 | 内容 |
| ---- | ---- |
| Pod 名称 | `JobsCustomView` |
| Pod 类型 | 自建本地 Pod |
| 版本 | `1.0.0` |
| 平台 | `ios 12.0` |
| 摘要 | Custom date picker view component for Jobs projects. |
| 首页 | [https://example.local/JobsCustomView](https://example.local/JobsCustomView) |
| 许可证 | `MIT` |
...
- 作为 Jobs 项目内的独立能力 Pod，向 App 或其它 Pod 提供 `JobsCustomView` 相关能力。
- 当 `JobsCustomView` 的 `Core`、`Support`、资源、依赖或公开头文件发生变化时，同步更新本 README，避免后续排查只看源码不看边界。
...
```text
JobsCustomView@Pods/
├── JobsCustomView.podspec  # Pod 描述文件
├── README.md  # 当前自述
├── JobsPodspecKit.rb  # 本地 podspec 基座
├── Core/  # 公开 API 与核心实现，2 个文件
└── Support/  # 内部支撑层，130 个文件
```

- `JobsCustomView.podspec` 是当前 Pod 的 [**CocoaPods**](https://cocoapods.org/) 描述入口。
...
- `Core` 当前包含 2 个文件，其中源码 / 头文件 2 个；按 Jobs 规范，它是 `JobsCustomView` 对外公开 API 和核心实现的边界。
- `Support` 当前包含 130 个文件，其中源码 / 头文件 129 个；它只服务当前 Pod 内部实现，不建议被 App 层或其它 Pod 直接引用。
...
- 如果 `JobsCustomView.h` 不是最终公开入口，请先修正 `JobsCustomView.podspec` 的 `public_header_files` 和入口头设计，再修改调用方。
...
修改 `JobsCustomView` 后，优先按风险从低到高验证：

```shell
ruby -c JobsCustomView.podspec
```

```shell
pod lib lint JobsCustomView.podspec --allow-warnings --verbose
```
