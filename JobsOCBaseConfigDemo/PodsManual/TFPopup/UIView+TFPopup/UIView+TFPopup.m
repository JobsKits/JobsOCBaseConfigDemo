...
-(void)tf_hide:(jobsByVoidBlock)completion {
    ...
    BOOL hasFrameAnimation = NO;
    BOOL hasBaseAnimation = NO;
    BOOL hasMaskAnimation = NO;
    ...
    if (self.extension.disuseHideAlphaAnimation == NO &&
        ...
        [UIView animateWithDuration:self.extension.hideAnimationDuration
            ...
            if (ext.hideAnimationCount == 0 && completion) completion(); // 在动画完成时调用completion
        }];
    }
    ...
    if (self.extension.disuseHideFrameAnimation == NO &&
        ...
        [UIView animateWithDuration:self.extension.hideAnimationDuration
            ...
            [weakself hideAnimationCompletion];
            if (ext.hideAnimationCount == 0 && completion) completion();
        }];
    }
    ...
    if ((hasAlphaAnimation || hasFrameAnimation || hasBaseAnimation || hasMaskAnimation) == NO) {
        if (removeIfNoAnyAnimation) {
            [self tf_remove];
            if (completion) completion(); // 在移除视图时调用completion
        }
    }
}
