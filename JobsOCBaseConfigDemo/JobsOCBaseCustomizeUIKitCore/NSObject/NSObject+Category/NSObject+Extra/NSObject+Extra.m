...
-(jobsByBtnBlock _Nonnull)jobsBackBtnClickEvent{
    ...
    return ^(__kindof UIButton *_Nullable sender) {
        ...
        if (KindOfVCCls(self)) {
            ...
            typedef JobsRetVCByViewBlock _Nullable (*JobsMsgSendVCBlockByVoid)(id, SEL);
            JobsRetVCByViewBlock getVCBlock = ((JobsMsgSendVCBlockByVoid)objc_msgSend)(self, @selector(getViewControllerByView));
            if (getVCBlock) vc = getVCBlock((UIView *)self);
        }
        if (!vc || ![self respondsToSelector:@selector(backViewControllerCore)]) return;
        typedef jobsByVCBlock _Nullable (*JobsMsgSendBackBlockByVoid)(id, SEL);
        jobsByVCBlock backBlock = ((JobsMsgSendBackBlockByVoid)objc_msgSend)(self, @selector(backViewControllerCore));
        if (backBlock) backBlock(vc);
    };
}
