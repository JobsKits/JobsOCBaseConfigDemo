...
+(JobsRetStrByStrBlock _Nonnull)stringByBase16String{
    return ^__kindof NSString *_Nullable(NSString *_Nullable base16StringByImage){
         ...
         */
        return NSString.initByUTF8Data(jobsMakeMutData(^(__kindof NSMutableData * _Nullable data) {
...
+(JobsRetImageByStrBlock _Nonnull)imageByBase16String{
    return ^UIImage *_Nullable(NSString *_Nullable base16StringByImage){
         ...
         */
        return UIImage.imageByData(jobsMakeMutData(^(__kindof NSMutableData * _Nullable imageData) {
            for (NSUInteger i = 0; i + 1 < base16StringByImage.length; i += 2) {
                NSString *hexChar = base16StringByImage.substringWithRange(NSMakeRange(i, 2));
                NSScanner *scanner = [NSScanner scannerWithString:hexChar];
                unsigned int intValue;
...
+(JobsRetDataByStrBlock _Nonnull)dataWithBase16String{
    return ^NSData *_Nullable(__kindof NSString *_Nullable base16StringByImage){
         ...
         */
        return NSData.initByData(jobsMakeMutData(^(__kindof NSMutableData * _Nullable data) {
            for (NSUInteger i = 0; i + 1 < base16StringByImage.length; i += 2) {
                NSString *hexChar = base16StringByImage.substringWithRange(NSMakeRange(i, 2));
                NSScanner *scanner = [NSScanner scannerWithString:hexChar];
                unsigned int intValue;
