...
```text
JobsIconfont
├── Core/                         # 公开门面、语义类型、加载与 UIKit 分类
├── Resource/JobsIconfontAssets.bundle
│   ├── JobsIconfontSample.ttf
│   ├── AlimamaShuzhiti-Regular.ttf
│   └── JobsIconfontCatalog.json
└── README.md
```

- `Core` 是唯一代码入口；公开层只暴露语义资源、图标枚举、字体 / 图片输出与加载结果。
- `Resource` 只保存框架内置资源和治理清单，不由业务层直接读取。
- URL、Unicode、字体内部名称、bundle 查找和 SDWebImage 配置均为私有实现。

## 依赖与引用

- 能力直接集成于主工程，复用工程已有 SDWebImage，不新增 Pod。
- 业务侧通过 `Demos.h` 与主工程头文件边界引用，不从 OC 新工程复制 Pod 形态。
- 框架内部动态注册字体，无需在业务工程的 `Info.plist` 维护 `UIAppFonts`。
...
- 修改源码或资源后检查 target 归属、资源 Copy Bundle Resources、主工程编译与 Demo 入口。
