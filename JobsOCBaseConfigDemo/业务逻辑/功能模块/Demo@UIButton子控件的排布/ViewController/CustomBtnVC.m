//
//  CustomBtnVC.m
//  JobsOCBaseConfigDemo
//
//  Created by Jobs on 2026年5月13日，星期三.
//

#import "CustomBtnVC.h"

@interface CustomBtnVC ()

Prop_strong()UIButton *btn1;
Prop_strong()UIButton *btn2;
Prop_strong()UIButton *btn3;
Prop_strong()UIButton *btn4;

@end

@implementation CustomBtnVC

- (void)dealloc{
    JobsLog(@"%@",JobsLocalFunc);
//    JobsRemoveNotification(self);
}

-(void)loadView{
    [super loadView];
    
    if ([self.requestParams isKindOfClass:UIViewModel.class]) {
        self.viewModel = (UIViewModel *)self.requestParams;
        if(self.viewModel.pushOrPresent != ComingStyle_Unknown){
            self.pushOrPresent = self.viewModel.pushOrPresent;
        }
    }
    self.viewModel
        .byBackBtnTitleModelBlock(^(__kindof UITextModel * _Nullable data) {
            data.byText(@"返回".tr);
        })
        .byTextModelBlock(^(__kindof UITextModel * _Nullable data) {
            data.byTextCor(HEXCOLOR(0x3D4A58));
        })
        //        self.viewModel.textModel.text = @"UIButton子控件的排布".tr;
        .byTextModelBlock(^(__kindof UITextModel * _Nullable data) {
            data.byText(data.attributedTitle.string);
            data.byFont(UIFontWeightRegularSize(16));
        })
    
        // 使用原则：底图有 + 底色有 = 优先使用底图数据
        // 以下2个属性的设置，涉及到的UI结论 请参阅父类（BaseViewController）的私有方法：-(void)setBackGround
        // self.viewModel.bgImage = @"内部招聘导航栏背景图".img;
        .byBgCor(RGBA_COLOR(255, 238, 221, 1))
        //    self.viewModel.bgImage = @"启动页SLOGAN".img;
        .byNavBgCor(RGBA_COLOR(255, 238, 221, 1))
        .byNavBgImage(@"导航栏左侧底图".img);
}

- (void)viewDidLoad {
    [super viewDidLoad];
    
    self.makeNavByAlpha(1);
    
    self.btn1.byAlpha(1);

    self.btn2.byAlpha(1);

    self.btn3.byAlpha(1);

    self.btn4.byAlpha(1);

}
#pragma mark —— lazyLoad
-(UIButton *)btn1{
    if (!_btn1) {
        _btn1 = UIButton.jobsInit()
            .jobsResetBtnTitle(@"image左、label右【系统默认状态】".tr)
            .jobsResetBtnImage(@"Lock".img)
            .jobsResetBtnTitleCor(JobsWhiteColor)
            .jobsResetBtnTitleFont(UIFontWeightRegularSize(14))
            .byImageView(^(UIImageView *imageView) {
                imageView.setLayerBy(jobsMakeLocationModel(^(__kindof JobsLocationModel * _Nullable model) {
                    model
                        .byJobsWidth(1)
                        .byLayerCor(HEXCOLOR(0xEEE2C8))
                        .byCornerRadiusValue(1);
                }));
            })
            /// 一行显示。不定宽、定高、定字体。宽度自适应 【单行：ByFont】
            .makeBtnTitleByShowingType(UILabelShowingType_03)
            .jobsResetImagePlacement_Padding(NSDirectionalRectEdgeLeading,JobsWidth(8))
            .byBgColor(JobsCyanColor)
            .addOn(self.view)
            .byAdd(^(MASConstraintMaker *make) {
                make.top.equalTo(self.gk_navigationBar.mas_bottom).offset(10);
                make.centerX.equalTo(self.view);
                make.height.mas_equalTo(JobsWidth(50));
            });
    };return _btn1;
}

-(UIButton *)btn2{
    if (!_btn2) {
        _btn2 = UIButton.jobsInit()
            .jobsResetBtnTitle(@"image右、label左".tr)
            .jobsResetBtnImage(@"Lock".img)
            .jobsResetBtnTitleCor(JobsWhiteColor)
            .jobsResetBtnTitleFont(UIFontWeightRegularSize(14))
            .byImageView(^(UIImageView *imageView) {
                imageView.setLayerBy(jobsMakeLocationModel(^(__kindof JobsLocationModel * _Nullable model) {
                    model
                        .byJobsWidth(1)
                        .byLayerCor(HEXCOLOR(0xEEE2C8))
                        .byCornerRadiusValue(1);
                }));
            })
            .jobsResetImagePlacement_Padding(NSDirectionalRectEdgeTrailing,JobsWidth(8))
            /// 一行显示。不定宽、定高、定字体。宽度自适应 【单行：ByFont】
            .makeBtnTitleByShowingType(UILabelShowingType_03)
            .byBgColor(JobsCyanColor)
            .addOn(self.view)
            .byAdd(^(MASConstraintMaker *make) {
                make.top.equalTo(self.btn1.mas_bottom).offset(10);
                make.centerX.equalTo(self.view);
                make.height.mas_equalTo(JobsWidth(50));
            });
    };return _btn2;
}

-(UIButton *)btn3{
    if (!_btn3) {
        _btn3 = UIButton.jobsInit()
            .jobsResetBtnTitle(@"image上，label下".tr)
            .jobsResetBtnImage(@"Lock".img)
            .jobsResetBtnTitleCor(JobsWhiteColor)
            .jobsResetBtnTitleFont(UIFontWeightRegularSize(14))
            .byImageView(^(UIImageView *imageView) {
                imageView.setLayerBy(jobsMakeLocationModel(^(__kindof JobsLocationModel * _Nullable model) {
                    model
                        .byJobsWidth(1)
                        .byLayerCor(HEXCOLOR(0xEEE2C8))
                        .byCornerRadiusValue(1);
                }));
            })
            /// 一行显示。不定宽、定高、定字体。宽度自适应 【单行：ByFont】
            .makeBtnTitleByShowingType(UILabelShowingType_03)
            .jobsResetImagePlacement_Padding(NSDirectionalRectEdgeTop,JobsWidth(8))
            .byBgColor(JobsCyanColor)
            .addOn(self.view)
            .byAdd(^(MASConstraintMaker *make) {
                make.top.equalTo(self.btn2.mas_bottom).offset(10);
                make.centerX.equalTo(self.view);
                make.height.mas_equalTo(JobsWidth(50));
            });
    };return _btn3;
}

-(UIButton *)btn4{
    if (!_btn4) {
        _btn4 = UIButton.jobsInit()
            .jobsResetBtnTitle(@"image下，label上".tr)
            .jobsResetBtnImage(@"Lock".img)
            .jobsResetBtnTitleCor(JobsWhiteColor)
            .jobsResetBtnTitleFont(UIFontWeightRegularSize(14))
            .byImageView(^(UIImageView *imageView) {
                imageView.setLayerBy(jobsMakeLocationModel(^(__kindof JobsLocationModel * _Nullable model) {
                    model
                        .byJobsWidth(1)
                        .byLayerCor(HEXCOLOR(0xEEE2C8))
                        .byCornerRadiusValue(1);
                }));
            })
            /// 一行显示。不定宽、定高、定字体。宽度自适应 【单行：ByFont】
            .makeBtnTitleByShowingType(UILabelShowingType_03)
            .jobsResetImagePlacement_Padding(NSDirectionalRectEdgeBottom,JobsWidth(8))
            .byBgColor(JobsCyanColor)
            .addOn(self.view)
            .byAdd(^(MASConstraintMaker *make) {
                make.top.equalTo(self.btn3.mas_bottom).offset(10);
                make.centerX.equalTo(self.view);
                make.height.mas_equalTo(JobsWidth(50));
            });
    };return _btn4;
}

@end
