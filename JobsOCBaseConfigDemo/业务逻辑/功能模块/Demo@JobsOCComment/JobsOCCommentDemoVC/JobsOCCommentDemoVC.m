...
-(jobsByJobsOCCommentModeBlock _Nonnull)jobs_showMode;
-(jobsByJobsOCCommentModeBlock _Nonnull)jobs_presentPopupMode;
-(jobsByJobsOCCommentModeBlock _Nonnull)jobs_pushMode;
-(JobsRetNSArrayJobsOCCommentModelByVoidBlock _Nonnull)jobs_makeDemoComments;

@end

@implementation JobsOCCommentDemoVC
-(void)loadView{
    jobsByVoidBlock action = ((jobsByVoidBlock (*)(__typeof__(self), SEL))JobsBlockInstanceMethodIMP(JobsOCCommentDemoVC.class, @selector(jobsLoadView)))(self, @selector(jobsLoadView));
    if (action) action();
}
...
-(void)viewDidLoad{
    jobsByVoidBlock action = ((jobsByVoidBlock (*)(__typeof__(self), SEL))JobsBlockInstanceMethodIMP(JobsOCCommentDemoVC.class, @selector(jobsViewDidLoad)))(self, @selector(jobsViewDidLoad));
    if (action) action();
}
...
-(__kindof UITableViewCell *)tableView:(UITableView *)tableView
    ...
    cell.textLabel
        .byText(JobsOCCommentConfig.titleByMode(mode).jobsTr())
        .byTextCor(JobsLabelColor)
        .byFont(UIFontWeightMediumSize(16));
    cell.detailTextLabel
        .byText(JobsOCCommentConfig.subtitleByMode(mode).jobsTr())
...
-(jobsByJobsOCCommentModeBlock _Nonnull)jobs_presentPopupMode{
    ...
    return ^(JobsOCCommentMode mode){
        ...
        vc.byMode(mode);
        vc.byPopupStyle(YES);
        vc.byCommentArr(self.commentArr);
        vc.byPopUpHeight(JobsMainScreen_HEIGHT() * 0.62);
        vc.byPushOrPresent(ComingStyle_PRESENT);
        vc.byPreferredContentSize(CGSizeMake(JobsRealWidth(), vc.popUpHeight));
        self.presentationController = [JobsPresentationCtrl.alloc initWithPresentedViewController:vc
                                                                         presentingViewController:presentingVC];
        vc.byTransitioningDelegate(self.presentationController);
...
-(JobsRetNSArrayJobsOCCommentModelByVoidBlock _Nonnull)jobs_makeDemoComments{
    @jobs_weakify(self)
    return ^NSArray<JobsOCCommentModel *> *{
        @jobs_strongify(self)
        if (!self) return nil;
