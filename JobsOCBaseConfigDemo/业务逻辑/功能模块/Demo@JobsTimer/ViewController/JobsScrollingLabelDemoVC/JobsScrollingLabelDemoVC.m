...
@interface JobsScrollingLabelDemoVC ()
...
@implementation JobsScrollingLabelDemoVC
    ...
    for (NSUInteger index = 0; index < titles.count; index++) {
        UIButton *button = jobsMakeButton(^(__kindof UIButton * _Nullable button) {
            button.jobsResetBtnTitle(titles[index])
                ...
                .byAddTarget(self, @selector(controlButtonTapped:), UIControlEventTouchUpInside)
                .byTag(index)
                .addOn(self.view);
        });
